...
| `scripts/check-no-naked-domain-strings.mjs` | scanner-backed by `TS-1.3` | Effect Schema brand direction is now in TypeScript source docs. |
...
| `scripts/check-no-tracked-generated-artifacts.mjs` | partially scanner-backed by `GEN-1.1` and `GEN-1.2` | Marker and generated-output-path checks exist; exact `git ls-files` tracked-only mode still needs a dedicated command. |
| `scripts/security/scan-staged-secrets.mjs` | partially scanner-backed by `SEC-1.1` and `SEC-1.2` | Inline secret-like assignments and sensitive file paths exist; staged-only mode is not yet equivalent. |
...
| `scripts/check-source-shape.mjs` | covered by `ocentra-enforcer check source-shape` and `SRC-1.1` | Config-driven policies support TypeScript and Rust file/function/export/type limits. |
...
| staged-vs-repo secret mode | partial only | Add explicit CLI/MCP modes for staged files and full repo scans. |
| tracked-only generated artifact mode | partial only | `GEN-1.2` catches generated output paths by scope, but old-vs-new parity still needs a `git ls-files` tracked-only comparison mode. |
| `scripts/check-import-boundaries.mjs` | partial only | Some boundaries are project-specific. Add a configurable import-boundary schema before moving this fully. |
