...
| [RUST_ARCHITECTURE.md](./RUST_ARCHITECTURE.md) | Governing architecture | The enforcer is a **RUST Cargo workspace** (one binary = MCP + CLI; rules structured-data; native Rust dogfood; TS only for Tauri UI; Node dropped). Crate map (**29 crates** = 25 arc + 3 E-built lang crates + 1 x06-built `enforcer-memory`), distribution (codebase-memory model), and the track re-cast. **Supersedes the `.mjs` -> TypeScript decision.** | Anyone orienting or building Track A | Read after PLAN_STATE |
...
118 assigned-workpack files. Each is a **scoped work item**, not an index — open only the one you have claimed (see the stop rule in every workpack capsule). Grouped:
...
| G UI layer (vendored hub dashboard/server) | `g01-*.md` .. `g09-*.md` | 9 | Human-invoked local UI built on the vendored `src/coordination/vendor/{server,dashboard}.js`: `g01` serve surface (lands first), then scan-report, per-violation actions, Run-dispatch into the coordination ledger, settings, hub dashboard, UI-security, rules-&-skills explorer, and read-only memory/KG/RAG explorer all mount into it. |
...
| B planning skill | `b01-*.md` .. `b06-*.md` | 6 | Scaffolder, structure validator, capsule/index templates, parallel-orchestrator binding, plan skill + self-validate, and `b06` the AGENTS.md decision-forest. |
| Cross-cutting | `x01-neutral-rename.md`, `x02-docs-refresh.md`, `x03-rename-migration.md`, `x04-main-branch-protection-ci.md`, `x05-lesson-capture-self-heal.md`, `x06-harness-memory-graph.md`, `x08-cross-harness-worklog.md`, `z01-dogfood-proof-gate.md` | 8 | `x01` renames the product to `enforcer` (early); `x02` refreshes product docs; `x03` migrates already-installed legacy registrations; `x04` protects main; `x05` captures orchestration lessons/self-heal; `x06` provides the harness memory graph; `x08` provides the cross-harness worklog read model; `z01` is the terminal dogfood-proof-gate (deps ALL tracks) that runs the enforcer on its own self and gates plan-DONE. |
