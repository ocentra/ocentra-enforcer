...
- **Cross-cutting (X), 4 packs:** early rename to `enforcer`; the terminal `z01` dogfood-proof-gate that runs the finished `enforcer` against its own multi-language self (bootstrap swap point) and gates plan-DONE on zero self-violations.
...
- **28 crates.** 25 `arc` crates (`enforcer-core`/`domain`/`config`/`rules`/`validator`/`lang-{rust,ts,py,common,security,iac,k8s}`/`literal-scan`/`mechanization`/`scan`/`coordination`/`proof`/`harness`/`security`/`plan`/`mcp`/`cli`/`install`/`ui`/`events`) + 3 lang crates built by Track E (`enforcer-lang-dart`, `enforcer-lang-cfml`, `enforcer-lang-crypto` [OPT-IN, off by default]). See the crate map in [`RUST_ARCHITECTURE.md`](./RUST_ARCHITECTURE.md).
