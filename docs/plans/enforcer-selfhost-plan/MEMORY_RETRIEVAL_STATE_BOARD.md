...
| X06.4 fulltext/vector/rerank | GATEKEEPING | `lane/x06-4-retrieval` @ bdb7ed4 | `proof/memory/x06-rag.json` (on lane) | Worker done: 120 tests claimed; D-07a=SQLite FTS5 (micro-benchmarked vs tantivy, decided on dep weight); deviations declared: soft-signal boost seam only, ort-models feature empty per D-03 |
