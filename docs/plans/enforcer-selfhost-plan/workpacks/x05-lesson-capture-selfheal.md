...
- [ ] `src/lessons.rs`: `LessonRecord { id: LessonId, date, observed, lesson, routes: Vec<LessonRoute>, landedAt: Vec<ArtifactRef> }` — serde camelCase, `LessonId`/`ArtifactRef` branded `enforcer-domain` newtypes, parse-at-boundary, no bare `String` ids. `LessonRoute` = enum `{ DoctrineBlock, Skill, RuleCandidate, ForestNode, PlanDoc }`.
...
- [ ] Import the seed corpus: a one-shot `lesson import` reads [refs/orchestration-lessons.md](../refs/orchestration-lessons.md) table rows into the ledger (L1..L11+), mapping their `ships-via` column to routes; import is idempotent (re-import adds nothing).
