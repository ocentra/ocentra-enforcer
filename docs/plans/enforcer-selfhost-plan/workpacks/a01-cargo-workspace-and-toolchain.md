...
### Current exact-SHA audit (2026-08-11)

At exact candidate SHA `7d832c0487093487d8573b55142cbea541dbab33`, the workspace manifests, pinned toolchain, deny policy, Windows-safe package-batched format boundary, security gates, workspace clippy, workspace build, focused rule-registry tests, and the `[lints] workspace = true` schema gate all pass in a clean E: worktree. The full-workspace test receipt is retained in `proof/cargo/a01-toolchain.txt`. This row remains `PENDING`, not GREEN, because the legacy Node/MJS package surface is explicitly deferred until the arc-21/arc-22/a10 cutover. The raw `cargo fmt --all --check` command still hits Windows OS error 206 inside the vendored tree; CI uses the checked-in bounded helper instead.
...
- [x] Root `Cargo.toml` declares `[workspace]` with `members = [ "crates/*" ]` (the crate map from RUST_ARCHITECTURE.md: `enforcer-core`, `enforcer-domain`, `enforcer-config`, `enforcer-rules`, `enforcer-validator`, `enforcer-scan`, `enforcer-cli`, ...) and `resolver = "2"`, plus `[workspace.package]`/`[workspace.dependencies]` for centralized versions.
- [x] `rust-toolchain.toml` pins a specific stable `channel` and lists `components = ["clippy", "rustfmt"]`, so `cargo clippy` and `cargo fmt` are reproducible.
- [x] Root `Cargo.toml` declares the `[workspace.lints]` deny-wall: `[workspace.lints.rust] unsafe_code = "forbid"` + a `[workspace.lints.clippy]` DENY set (`unwrap_used`, `expect_used`, `panic`, `todo`, `unimplemented`, `dbg_macro`, `print_stdout`, `print_stderr`, `await_holding_lock`, `future_not_send`, `clone_on_ref_ptr`, `redundant_clone`, `needless_pass_by_value`, `map_err_ignore`, `large_enum_variant`). Each member crate opts in with `[lints] workspace = true`. The ONLY `print_stdout`/`print_stderr` escape hatch is a scoped `#[allow(...)]` reserved for a single output-sink module in `enforcer-cli` and one in `enforcer-mcp`.
- [x] `deny.toml` configured for `cargo deny check` (advisories + licenses + bans + sources); `cargo audit` wired in CI.
...
- [x] `.github/workflows/ci.yml` (the base gate a01 owns) runs the full gate on push/PR across the target matrix: the Windows-safe bounded formatter `node scripts/check-cargo-workspace-members.mjs --fmt-check` (equivalent package checks; raw `cargo fmt --all --check` is OS206 on this vendored tree), `cargo clippy --all-targets --all-features -- -D warnings`, `cargo test --workspace`, `cargo deny check`, and `cargo audit`. (The separate native self-enforcement workflow `.github/workflows/dogfood.yml` is owned by a10, not a01.)
- [x] The workspace-member boundary test rejects any product crate that drops `[lints] workspace = true`; `tests/cargo-workspace-members.test.mjs` proves the seeded missing-opt-in case fails, and the bounded CI helper validates all 30 current product manifests.
- [x] `cargo build --workspace` succeeds on the skeleton tree; the retained seeded `clippy`/`fmt` fixtures make the corresponding package gates exit non-zero.

## Acceptance And Proof
Tier P1. A CI/test row in TEST_PROOF_EXPECTATIONS.md asserts each gate's exit code: the Windows-safe bounded formatter, workspace-member lint-opt-in schema check, `cargo clippy -D warnings`, `cargo test --workspace`, `cargo deny check`, and `cargo audit` all exit 0 on the clean skeleton and non-zero against seeded violation fixtures (missing lint opt-in / unformatted file / clippy lint / vulnerable dep pin). A seeded `unwrap()` (or an unscoped `println!`) in a member crate makes `cargo clippy` exit non-zero, proving the deny-wall is live and workspace-wide. The remaining acceptance gap is the eventual package.json runtime cutover; a01 remains PENDING until that dependency-legal cutover is separately evidenced.
