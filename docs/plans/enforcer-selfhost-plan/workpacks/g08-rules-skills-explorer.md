...
- [ ] **Dual-audience render** (owner req 2026-07-04): the UI renders BOTH the human-VERBOSE form AND the AI-DENSE form of each rule and skill — a toggle or dual-pane view — so a human can read either the full explanation or the ultra-dense summary the AI consumes. Both forms are projected from the same typed source; they are not two hand-maintained texts.
- [ ] **Typed source of truth / `.md`-as-TRANSITIONAL** (owner req 2026-07-04): the underlying data is TYPED (rules-as-data — the `enforcer-rules` arc-04 record IS the source of truth); any `.md` is a RENDERED VIEW, not the source. The explorer projects both the verbose and dense forms from the typed record, demonstrating the `.md`-is-droppable principle (a future typed system/db/schema replaces the `.md` with no change to the browse payload). The explorer must NOT read the `.md` as canonical for rule data.
