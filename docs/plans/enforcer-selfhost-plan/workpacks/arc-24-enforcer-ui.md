...
## Where We Want To Be
`enforcer-ui` is the Track G crate per RUST_ARCHITECTURE.md: it stands up the crate SKELETON — the UI server / Tauri backend (Rust) serving a self-contained HTML fallback for headless use and backing the Tauri desktop app, plus the Rust->TS type-generation pipeline. The frontend is TS/web living under `crates/enforcer-ui/frontend/` (the sole TS in the product, presentation only); the backend calls `enforcer-scan`/`enforcer-mcp`/`enforcer-memory` through Rust handlers — no business logic in TS. Frontend types are DERIVED from `enforcer-domain` via `#[derive(ts_rs::TS)]` -> an export bin/xtask -> committed generated `.ts`, guarded by a fail-closed `cargo test` drift test (byte-compare committed vs freshly-emitted). It hosts the Track G feature modules (g01-g09).
...
- [ ] Enforce the UI-domain boundary: TypeScript renders Rust-provided payloads, keeps transient presentation state, and emits user intents only; graph/RAG/model/learning decisions, retrieval ranking, validation, mutation, dispatch, coordination, and policy semantics live in Rust handlers.
...
Parallel Ownership Note (disjoint feature packs): the Track G feature packs each own SPECIFIC modules under this crate — g01 `src/serve.rs` (Tauri shell + served HTML fallback); g02 `src/report/`; g03 `src/actions/`; g04 `src/run_dispatch/` (deps arc-16); g05 `src/settings/` (config control-plane, writes routed through arc-23 c-track adapters); g06 `src/hub/` (live lane/claim/lease/mail panel); g07 `src/security/`; g08 `src/explorer/` (rules-&-skills explorer, where the human-canonical `.md` is browsed — the AI still reads the structured rule); g09 `src/memory_explorer/` + `tests/memory_explorer/**` (read-only x06 memory/KG/RAG/model/learning explorer) — each with its own frontend assets and fixtures. They own their modules, NOT the whole crate; they `deps:` arc-24 and are sequenced after this skeleton. owns stay DISJOINT BY FILE. Parallel-safe with arc-23 (install) — disjoint crate trees. Last surface; presentation only.

The app target is a Rust/Tauri desktop app plus the served fallback, supported on Windows, macOS, and Linux. UI completion requires actual click-through proof: Playwright or equivalent browser automation over the served fallback, plus Tauri/manual or automated desktop smoke where available, must exercise the real UI interactions against Rust handlers rather than unit-testing TS logic in isolation.
