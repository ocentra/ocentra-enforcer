...
- [x] **no-repodep-in-routers** (`py-fastapi-no-repo-in-routers`, T1, symbol-level): a `routers/**` module referencing a `*Repository` symbol is flagged; router depending on a service stays clean. Shipped as `PYFA-1.1` (`NoRepoInRoutersValidator`).
- [x] **no-session-in-services** (`py-fastapi-no-session-in-services`, T1): a `Session`/`AsyncSession` param or use inside `services/**` is flagged; service taking a repo stays clean. Shipped as `PYFA-2.1` (`NoSessionInServicesValidator`).
- [x] **no-transaction-in-services** (`py-fastapi-no-transaction-in-services`, T1): `commit()`/`begin()`/`session.rollback()` in `services/**` flagged; tx owned at boundary/unit-of-work stays clean. Shipped as `PYFA-3.1` (`NoTransactionInServicesValidator`).
- [x] **no-models-in-services** (`py-fastapi-no-orm-models-in-services`, T1): importing ORM model classes into `services/**` flagged; service using domain DTOs stays clean. Shipped as `PYFA-4.1` (`NoOrmModelsInServicesValidator`).
- [x] **no-sqlalchemy-in-routers** (`py-fastapi-no-sqlalchemy-in-routers`, T1): `from sqlalchemy` / `select(`/`.query(` in `routers/**` flagged; router delegating stays clean. Shipped as `PYFA-5.1` (`NoSqlalchemyInRoutersValidator`).
- [x] **HTTPException-outside-handlers** (`py-fastapi-httpexception-location`, T1): `raise HTTPException` outside `routers/**` (e.g. in services/domain) flagged; raised only in routers stays clean. Shipped as `PYFA-6.1` (`HttpExceptionLocationValidator`).
- [x] **no-repos-in-workflows** (`py-fastapi-no-repos-in-workflows`, T1): a `workflows/**` module using a repository directly flagged; workflow calling a service stays clean. Shipped as `PYFA-7.1` (`NoReposInWorkflowsValidator`).
- [x] **models-use-Mapped** (`py-fastapi-models-mapped`, T1): a SQLAlchemy model column not typed with `Mapped[...]` flagged; `Mapped[int]`/`mapped_column` stays clean. Shipped as `PYFA-8.1` (`ModelsMappedValidator`).
- [x] **StrEnum-only + enum-discipline** (via d16 `py.enum.strenum-only`/`py-fastapi-enum-location`, T1): `class X(Enum)` or non-StrEnum in `enums/` flagged; `class X(StrEnum)` under `enums/` clean. Consumed read-only from d16's `FSM-ENUMLOC.1` (`enforcer-lang-common::rules::fsm::EnumLocationStrEnumOnlyValidator`) — no new record minted by this pack.
- [x] **domain-purity / domain-no-ui-exceptions** (`py-fastapi-domain-purity`, T1): `domain/**` importing FastAPI/HTTP or raising `HTTPException` flagged; pure domain stays clean. Shipped as `PYFA-9.1` (`DomainPurityValidator`).
- [x] **no-sync-http** (`py-fastapi-no-sync-http`, T1): `requests.`/sync `httpx.Client` in async request path flagged; `httpx.AsyncClient`/`await` stays clean. Shipped as `PYFA-10.1` (`NoSyncHttpValidator`).
- [x] **no-direct-repo-instantiation** (`py-fastapi-no-direct-repo-instantiation`, T1): `SomeRepository(...)` constructed inline flagged; injected via `Depends`/DI stays clean. Shipped as `PYFA-11.1` (`NoDirectRepoInstantiationValidator`).
- [x] **python security** (`py-fastapi-plaintext-password`, `py-fastapi-insecure-random-token`, `py-fastapi-cors-wildcard`, T1): plaintext password store/compare, `random.*` for a token, and `allow_origins=["*"]` flagged; bcrypt/argon2, `secrets.token_hex`, explicit origin list clean. Shipped as `PYFA-12.1`/`PYFA-12.2`/`PYFA-12.3`.

Size/shape caps (nesting depth, catch-all utils) are likewise consumed read-only from d22's `SIZE-NEST-1.1` (`enforcer-lang-common::rules::size_shape`) — no new record minted by this pack. All 14 `PYFA-*` records live in `crates/enforcer-rules/rules/fastapi-layered.json` (`RuleId` requires an uppercase prefix, so the workpack's lowercase `py-fastapi-*` names are carried as `legacyIds` in each record's `params`). Proof: `proof/rules/e-pack-python.txt`.
