...
## Where We Want To Be
A one-time transitional migration module `crates/enforcer-install/src/migrate_legacy_name.rs` (invoked by the installer's `doctor`/`migrate` path via arc-22 CLI) that: detects any existing `ocentra-enforcer` MCP registration across all supported harness config locations plus legacy `rust_rules_*` / `ocentra_enforcer_*` tool-name usages; rewrites the registration to `enforcer` (tools resolve as `mcp__enforcer__*`, reading the server-name const x01 owns); drops the deprecated aliases with a single one-time migration notice; RETIRES the legacy `skills/rust-rules-hard-gate/` source tree (including its `agents/openai.yaml` descriptor) now that the canonical `enforcer` skill + native per-adapter descriptors exist; and reports exactly what changed as a structured c01/arc-23 `Report`/`ApplyResult` record (typed `Finding`s, not println). It reuses the arc-23 config-read/JSON-and-TOML-merge + managed-block/backup helpers rather than reimplementing config parsing. This is transitional migration, NOT a permanent alias — after migration, zero `ocentra-enforcer` entries remain.
...
- [ ] RETIRE the legacy `skills/rust-rules-hard-gate/` skill dir (incl. its `SKILL.md` and `agents/openai.yaml` — the `rust-rules-hard-gate` / `rust_rules_*` alias descriptor) once the canonical `enforcer` skill (x02) + the native per-adapter descriptors (c03 Claude `.claude/agents/*.md`, c06 Codex `agents/openai.yaml`) exist. This is a transitional cleanup — delete the legacy source tree, do NOT keep it as a permanent alias. A post-migration re-scan finds no `skills/rust-rules-hard-gate/` and no `rust-rules-hard-gate` descriptor. (x02's `skills/*/SKILL.md` glob only REFRESHES surviving skills; the retirement of this specific legacy dir is owned here.)
- [ ] No permanent alias retained — a post-migration re-scan finds zero `ocentra-enforcer` entries and zero `rust-rules-hard-gate` skill/descriptor residue; back up each config with a timestamped copy before rewrite. Obey `[workspace.lints]` (no `unwrap`/`expect`/`panic`/`print_*`; fail-closed typed error on malformed config); no `pub use` barrels.
...
- **Legacy-skill-retirement test** `legacy-skill-retired`: assert that after migration the legacy `skills/rust-rules-hard-gate/` dir (incl. `agents/openai.yaml`) is gone and a repo/asset scan finds zero `rust-rules-hard-gate` skill/descriptor residue, while the canonical `enforcer` skill remains.
