...
- [x] Detect a fixed vocabulary of deferral markers per target language (TODO, FIXME, stub throws, `unimplemented!`, `todo!`, `raise NotImplementedError`, `pass  # TODO`) — string/marker scan over scanned files, wired through the `enforcer-validator` trait (arc-05). Implemented as `DEFERRAL_MARKERS` + `find_marker` in `crates/enforcer-lang-common/src/rules/deferred_work.rs`.
- [x] Exempt only markers matching the exact `DEFERRED(#<ref>)[revisit:<value>]` grammar (a parse-at-boundary parser, typed error); malformed annotations still emit an error `Finding`. Implemented as `parse_deferred_annotation` + `DeferredAnnotationError` (`crates/enforcer-lang-common/src/error.rs`); malformed and unmarked are distinct `Finding` titles.
- [x] Diff-scoped: only lines added/changed in the working diff are gated, so legacy stubs do not block (composes with d02 baseline). Diff scope comes from the `enforcer-scan` run context. This crate's `Validator` is pure/per-file per the harness contract (same as every other validator in this workspace); file-level diff scoping composes at the `enforcer-scan` layer (`ScanScope::Diff`) — documented in the module's doc comment.
- [x] `#<ref>` must be non-empty; `revisit:` value must be non-empty. Enforced by `parse_deferred_annotation`; proved by `malformed_annotation_still_fails` and `missing_revisit_still_fails`.
- [x] Emitted as a first-class rule record via d01 (record + fixtures + doc-anchor + `Validator` parity); obey `[workspace.lints]` (no `unwrap/expect/panic/print_*`). Record: `crates/enforcer-rules/rules/deferred-work-gate.json` (`DEFER-1.1`), wired into `crates/enforcer-rules/tests/registry_load.rs`; clippy clean.
