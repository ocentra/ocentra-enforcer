...
- [ ] **Dual-audience authoring** (owner req 2026-07-04): the `plan` skill AUTHORS both audiences for any rule/skill/plan doc it generates — a human-readable VERBOSE form AND a super-high-information-density AI-optimized TERSE form (single doc with delimited dense-summary + verbose-body, or paired dense companion). The scaffolder output it wires (b01) emits both; `SKILL.md` itself ships in both forms; the AI-dense form is authored to map onto a future typed record (`.md`-as-TRANSITIONAL — droppable for a typed system/db/schema, humans reading via the Tauri UI). A doc-parity check asserts every generated/authored doc carries both forms.
- [ ] Obeys `[workspace.lints]` for the Rust it owns (no `unwrap`/`expect`/`panic`/`print_*`); no `pub use` barrels.

## Acceptance And Proof
Tier P4 (self-enforce green), T1, Rust-native (`cargo test`). Prove via `cargo test -p enforcer-plan` (self-validate) and `cargo test -p enforcer-install` (command emitter): the self-validate test (`crates/enforcer-plan/tests/self_validate.rs`) invokes the b02 `Validator` entrypoint against this plan dir and asserts zero PLAN-* `Finding`s; a command-emitter test asserts the `commands/plan.rs` emitter produces a `/plan` command that dispatches to the real scaffolder + validator via the `enforcer` binary (not a stub); a doc-parity check asserts every doctrine claim in `SKILL.md` cites a concrete `Validator`/`ruleId`; and a dual-audience check asserts every doc the skill authors/generates (and `SKILL.md` itself) carries BOTH a human-verbose form and a delimited AI-dense form. Name these detection tests in TEST_PROOF_EXPECTATIONS.md.
