...
- [ ] PLAN-RESUME-STATE: every plan carries live resume-state (owner req 2026-07-04, AUDIT_FINDINGS WAVE 5) — a `RESUME_STATE.md` (or the equivalent required section-set) with a `Where We Are` block AND `CHECKLIST` / `TASKLIST` / `PROGRESS` lists AND `PREV` / `NEXT` records (done / in-progress / next). A `Validator` impl emits a `Finding` when the file/section-set is missing OR any of Where-We-Are / checklist / tasklist / progress / prev / next is absent, so resume-state is mechanically required, not optional. Fail fixture: a plan missing Where-We-Are or any resume record. Pass fixture: a plan with the complete set.
...
Tier T1 / P4 (self-enforce green), Rust-native (`Validator` impls + fail/pass fixtures + `cargo test` detection). Prove via `cargo test -p enforcer-plan` (and `-p enforcer-rules` for record parity): a rules detection test drives each PLAN-* rule against pass/fail fixtures in `crates/enforcer-plan/tests/fixtures/plan-validator/**` (including PLAN-RESUME-STATE: a fail fixture whose plan omits `RESUME_STATE.md`/Where-We-Are/a resume record must yield a `Finding`, a complete pass fixture must yield none); a self-host test runs the validator against `docs/plans/enforcer-selfhost-plan/` and asserts zero `Finding`s (self-enforce green); a parity test asserts every PLAN-* `ruleId` maps to a rule record, a `Validator`, a doc anchor, and both fixtures. Name these detection tests in TEST_PROOF_EXPECTATIONS.md.
