...
- [ ] Ship the `ocentra-parent` profile's specific Rust posture as an EXPLICIT rules-as-data PORT TARGET (typed rule records the registry ships — not left implicit): (a) pub-use forbidden repo-wide → `publicReexportPolicy: forbid` (the record data paired with arc-06's `no-reexports` Validator by `RuleId`); (b) runtime-literal ban; (c) domain-typed serialized public fields (public serialized fields must be `enforcer-domain` branded newtypes, no bare primitives on the wire); (d) protocol-crate dependency bans → `blockedProtocolDependencies` (crates a protocol crate may not depend on). Each becomes a typed rule record with `ruleId <-> validator <-> {fail+pass fixtures} <-> doc-anchor <-> tier`. (WAVE 6: `ocentra-parent` profile port must be named here, not implicit.)
- [ ] This registry CONSUMES arc-03's `sourceShapePolicies` base shape (homed in arc-03's `enforcer-config`) as the substrate for the ocentra-parent posture records — reference it, do NOT redefine/re-home the base shape here.
...
Tier P1. Proof row asserts `cargo test -p enforcer-rules` exits 0 — registry loads, all rule records validate (fail fixture for a malformed/duplicate rule), fixture/doc-anchor references resolve, the deny-wall + no-reexports T1 rule records load, and d13 version-drift fails closed on a seeded drift (rule version bumped without matching fixture/anchor). The `ocentra-parent` posture resolves to the expected rule records (fixture: loading the profile yields the `publicReexportPolicy: forbid`, runtime-literal-ban, domain-typed-serialized-fields, and `blockedProtocolDependencies` records with the expected ids/validators/fixture refs; a missing/renamed posture record fails closed). Record the artifact path in TEST_PROOF_EXPECTATIONS.md.
