...
- [ ] Implement [MEMORY_RETRIEVAL_QA_PROOF_GATE](../MEMORY_RETRIEVAL_QA_PROOF_GATE.md) (owner-set REQUIRED gate): all 100 rows executable, each recording expected ids / actual top-k / Recall@5 / MRR@10 / nDCG@10 / reranker lift / token-reduction estimate / source refs / verdict into `proof/memory/x06-rag-qa.json`; minimum pass Recall@5>=0.90, MRR@10>=0.80, nDCG@10>=0.85, zero unauthorized candidates, zero untrusted active lessons, zero exact-artifact mismatches. The QA-250 expansion (MEMORY_RETRIEVAL_QA_BENCHMARKS §2) extends this gate, never replaces it.
