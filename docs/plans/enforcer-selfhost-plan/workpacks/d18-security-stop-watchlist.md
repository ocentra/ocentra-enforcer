...
- owns: `rules/common/security-stop.md, src/validators/security-stop-*.ts, tests/fixtures/security-stop/**`
- deps: `d01`
- tier: `P0`
...
## Where We Are
The registry has generic entropy/secret rules and python-shaped eval/pickle bans, but the real-time CWE STOP watchlist ADBP mandates (rows 68-81, 103 of ADBP_GAPS.md) is not backed as a deterministic pattern/AST family: no SQL-injection f-string ban, no path-traversal canonicalization gate, no weak-crypto ban, no CSPRNG-for-tokens gate, no constant-time-compare gate, no plaintext-password ban, no CORS-wildcard ban, no error-leak-to-client ban, no untrusted-deserialize/shell-exec gate, and no frontend XSS/localStorage-token/`NEXT_PUBLIC_`-secret bans.

## Where We Want To Be
A cross-language security STOP family (docs in `rules/common/security-stop.md`, validators `src/validators/security-stop-*.ts`, fixtures `tests/fixtures/security-stop/**`), scaffolded via d01 with full 5-way parity. These are deterministic pattern/AST detections and block (T1).

## Requirement Checklist
Each rule is scaffolded with `enforcer rule new <ID>` (d01), landing a doc anchor, a `security-stop-*.ts` validator, and a fail+pass fixture pair under `tests/fixtures/security-stop/`.
...
Tier P0. The T1 rules are deterministic pattern/AST detections and block; the two audit/supply-chain rows are T2 scored (fixtures assert the score crosses the fail threshold and stays under it on pass). The only T3 residue is the subagent runtime-honoring of the STOP interrupt, carried with the label `advisory, no mechanization possible + runtime LLM behavior is unobservable to a static gate`; the label's presence is T1-enforced and the prompt-assembly half of SEC-STOP-GATE is fully T1. For every ruleId the fail-fixture is flagged and the pass-fixture stays clean under its `security-stop-*.ts` validator; the detection test asserts both. Re-run the d01 `rule-scaffold-parity` oracle and record detection-test artifact paths in TEST_PROOF_EXPECTATIONS.md.

Representative triples:
- py.sec.sql-injection-fstring: fail `tests/fixtures/security-stop/sqli/fail_fstring.py`, pass `.../pass_parameterized.py`, test `sec-sqli.test`.
- SEC-WEAK-HASH: fail `tests/fixtures/security-stop/weak-hash/fail_md5.rs`, pass `.../pass_sha256.rs`, test `sec-weak-hash.test`.
- SEC-STOP-GATE (prompt-assembly T1): fail `tests/fixtures/security-stop/stop-gate/fail_prompt_missing_block1.txt`, pass `.../pass_prompt_with_block1.txt`, test `sec-stop-gate.test`.

## Parallel Ownership Notes
Owns `rules/common/security-stop.md`, `src/validators/security-stop-*.ts`, and `tests/fixtures/security-stop/**` exclusively; disjoint from siblings. Depends on d01. Distinct from the Rust error-handling pack (d17) and the FSM pack (d16); the frontend XSS/token/env rows here are the security-only slice and must not overlap the broader `FE-*` family owned by e-pack-frontend-react. The SEC-STOP-GATE prompt-assembly rule shares the dispatch-prompt surface with d26-dispatch-prompt-assembly (DISP-1.1); this pack owns only the SECURITY-STOP block content/presence check, d26 owns block ordering.
