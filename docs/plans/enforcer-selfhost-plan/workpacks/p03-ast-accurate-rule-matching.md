...
Execution refinement: [Universal Language Enforcement](../../universal-language-enforcement-plan/README.md) UL02-UL05 supersede the TypeScript-only optional dependency on the entire `enforcer-memory` crate. Grammar ownership is transferred once, a deep `enforcer-syntax` module is extracted for all consumers, parse quality/facts become explicit, and validators receive a shared parse-once analysis seam. The false-positive pilot proposed here may proceed only as UL08 after that foundation; p03 does not create a second AST provider or import memory persistence into a language rule crate.
