...
## Where We Want To Be
A `crates/enforcer-install/src/adapters/codex.rs` module implementing the arc-23 `Adapter` trait (`plan`/`apply`/`verify`) that produces the same on-disk result as the reference Codex integration, driven through the c01 install orchestrators. TOML is edited via `toml_edit` (format-preserving); the `AGENTS.md` managed block and backups are emitted by shared arc-23 core helpers. It emits the Codex agent descriptor (`agents/openai.yaml`: `display_name`/`short_description`/`default_prompt`/`allow_implicit_invocation`) NATIVELY from the adapter — not via the legacy bulk `.codex-plugin` publish (`plugin.json "skills": "./skills/"`), which dragged the whole `skills/` tree along — so the descriptor is a first-class, per-adapter emitter mirroring c03's Claude descriptor. It also owns the harness-neutral GLOBAL `AGENTS.md` managed-block behavior (the `<!-- ocentra-enforcer:start -->` / `<!-- ocentra-enforcer:end -->` markers), the user-skill copy, and the global `OCENTRA_LEDGER_HOME` config — the global-side behaviors that were Codex-only in the legacy `.mjs` code.
...
- [ ] EMIT the Codex `agents/openai.yaml` agent descriptor NATIVELY from the adapter (`display_name`/`short_description`/`default_prompt`/`allow_implicit_invocation`, keyed to the x01-owned server-name const) — NOT via the legacy bulk `.codex-plugin` publish (`plugin.json "skills": "./skills/"`). Collapse that bulk publish so the descriptor is a per-adapter emitter, matching how c03 emits the Claude descriptor.
- [ ] Own the harness-neutral GLOBAL `AGENTS.md` managed-block: upsert the `<!-- ocentra-enforcer:start -->` / `<!-- ocentra-enforcer:end -->` block into the global `AGENTS.md` via the c01/arc-23 marker/managed-block helpers, copy the enforcer user-skill into the global skills location, and write the global `OCENTRA_LEDGER_HOME` config — the global-side behaviors that were Codex-only in the legacy `.mjs` code. (Note: the marker text is the transitional `ocentra-enforcer` literal that x03 migrates; keep it byte-for-byte until then.)
...
P5 parity (`codex-adapter-parity` in TEST_PROOF_EXPECTATIONS.md), proved by `cargo test -p enforcer-install`: golden-file `#[test]`s assert the adapter's generated TOML block, the native `agents/openai.yaml` descriptor, and the GLOBAL `AGENTS.md` managed-block (start/end markers + body) each equal pinned snapshots under `crates/enforcer-install/tests/fixtures/codex/`; a fixture also asserts the global user-skill copy + `OCENTRA_LEDGER_HOME` config are written, and that the descriptor comes from the native emitter (no reliance on the bulk `.codex-plugin` publish). Doctor check names/severities match a pinned snapshot. Any diff fails the build.
