...
## Where We Want To Be
Stand up `crates/enforcer-ui/src/serve.rs`: the first-class serve surface exposing `enforcer serve` / `enforcer ui` (clap CLI in `enforcer-cli`, arc-22) and an `mcp__enforcer__ui` tool (arc-21). It drives the arc-24 UI server — the Tauri shell for the desktop app AND the served self-contained HTML fallback for headless use — loopback (127.0.0.1) default, token required for any non-loopback bind. It provides a neutral shell + a Rust-side VIEW-MOUNT REGISTRY that g02 (report), g05 (settings), g06 (hub), g08 (explorer), and g09 (memory/KG/RAG explorer) MOUNT into. Presentation-only TS lives under `crates/enforcer-ui/frontend/` (types derived from `enforcer-domain` via `ts_rs`); no business logic in TS. This is HUMAN-invoked surface only; inline agent checks stay silent (see f04, `enforcer-core` run-context).
...
- [ ] Treats the serve surface as the Rust-owned interaction boundary: TS click handlers may collect presentation intent only, then call Rust commands/endpoints that validate, decide, mutate, retrieve, rank, dispatch, and return the next render payload.
...
Owns `crates/enforcer-ui/src/serve.rs` exclusively; does not own the arc-24 crate skeleton (`Cargo.toml`/`lib.rs`/backend root — read/drive only). Foundation for g02/g05/g06/g08/g09 — they register views into the registry, they never re-open the transport. g09 owns the implementation under `crates/enforcer-ui/src/memory_explorer/`; g01 only provides the neutral shell/registry entry and must not implement graph/RAG/model/learning logic itself. Deps arc-24 (crate skeleton) and is sequenced after it exists. owns stay DISJOINT BY FILE from sibling g0x modules.

The served fallback and Tauri frontend must share the same Rust command semantics so Windows/macOS/Linux behavior is proven once at the Rust boundary and smoke-tested through the UI shell.

Proof must include real click-through automation for the served fallback (Playwright or equivalent): open the UI, click mounted views/buttons, submit at least one action intent, assert the Rust handler response is rendered, and assert no external assets or silent-agent popup behavior. Desktop/Tauri smoke proof is required where the platform runner is available.
