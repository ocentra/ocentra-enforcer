...
"Ready now" = every dep is DONE **and** the pack's `owns:` set is disjoint from all currently-claimed lanes. The graph currently imports **118 workpacks**, with **8 DONE** and **110 TODO**. Its self-host ready frontier is `a01` and `g01`; `d15` is already DONE. The cross-program graph also reports manager-owned `CP08` and `UL00` as ready, but this plan must not claim those lanes. Everything else remains dependency-derived and blocked until its named prerequisites are DONE. **Track A is re-cast to RUST** (see [RUST_ARCHITECTURE.md](./RUST_ARCHITECTURE.md)): the crate-build swarm `arc-01`..`arc-25` (25 crate packs, incl. the vendored `arc-25` `enforcer-events`) replaces the removed 50-pack `.mjs`->TS conversion swarm; `a01` owns the workspace root and precedes `arc-01` (`enforcer-core`, the first member crate), with `arc-02` (`enforcer-domain`) the schema keystone. With the plan now at **118 workpacks** (Track A = 35; Track C = 11; Track D = 25; Track E = 6; Track B = 6; Track F = 5; Track G = 9; Track H = 10; cross-cutting/X = 5), `z01` remains the terminal dogfood-proof gate that only becomes claimable once every other required pack is DONE. The crypto pack is OPTIONAL/opt-in (OFF by default), never on the default frontier. Tracks F and G ride the foundation: `g01` (UI serve surface, dep `arc-24`) and `f01`/`f03` are early leaves once their dependencies are DONE; within G, `g01` must land before the remaining UI packs mount into/guard it. `f05` (detect-and-route) is the foundational router consumed by the scan/check/run surfaces.
