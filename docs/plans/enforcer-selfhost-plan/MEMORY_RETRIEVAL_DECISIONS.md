...
## D-17 — Portable model-runtime QA markers — LOCKED
Context: QA rows `QA-061` and `QA-062` verify the backend and no-remote-model policy through this decision ledger, while executable proof lives in `crates/enforcer-memory/src/local_runtime.rs`, `crates/enforcer-memory/src/model_runtime.rs`, and `proof/memory/x06-models.json`. Decision: the laptop/runtime policy is Windows-first with CPU/GPU/NPU routing; GPU and NPU claims require provider probes, cache validation, and explicit runtime telemetry before they can be treated as loaded providers. `ort` offers DirectML/OpenVINO execution providers behind explicit cache/provider selection, while llama.cpp/GGUF remains the first-class chat/runtime path. Enforcer uses hardware detection/fallback ordering harvested from TabAgentServer `execution-providers`, adapted to Enforcer-owned runtime admission rather than copied as a server dependency; the default build remains deterministic and offline; local model loading is cache-first, no remote model provider is used implicitly, and missing model/provider/tokenizer artifacts degrade to provider-unavailable instead of downloading or pretending parity. Consequence: QA may treat these policy rows as runnable degraded evidence, but real model parity still requires the separate GGUF/ORT execution artifacts and cannot be inferred from this ledger alone.
