...
## D-03 — Local inference backend: ONNX Runtime (`ort`) behind a trait seam; candle is the named fallback — RESOLVED (2026-07-06)
Outcome as shipped by the Codex model-runtime harvest (merged c9f4d13): `ort` carries the Qwen3 embedding + reranker path (CPU proven, Vulkan probed) AND a `llama.cpp` backend landed alongside for the generative/chat-model path — both behind opt-in features (`ort-models`, `model-downloads`, `real-models`); the default build remains deterministic and offline. candle was not needed. Proofs: `proof/memory/x06-models*.json`, `x06-retrieval-quality.json`.
Original decision text (context preserved):
Context: OcentraParent has NO production runtime (scout-verified 2026-07-04; corrects DESIGN_INPUTS Source 3) — x06 builds its own inference layer. Laptop contract is Windows-first with CPU/GPU/NPU routing; `ort` offers DirectML/OpenVINO execution providers and mature Qwen3-class embedding/reranker ONNX availability. Decision: X06.4 implements `Embedder`/`Reranker` traits; default impl uses `ort`; capability-state contract adopted from OcentraParent shapes (`LoadState: unavailable|loading|loaded|degraded|failed`, `ResourceClass: cpu|gpu|npu`, typed degraded reasons); hardware detection/fallback ordering harvested from TabAgentServer `execution-providers`. Revisit-trigger: if `ort` dylib distribution or Windows CI proves unworkable, switch default to candle and record here. Consequence: `proof/memory/x06-models.json` records backend, model ids, quant, provider, and capability state per run; degraded mode is labeled and is NOT accepted for feature parity.
