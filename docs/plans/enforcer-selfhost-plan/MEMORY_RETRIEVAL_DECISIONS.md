...
## D-03 — Local inference backend: ONNX Runtime (`ort`) behind a trait seam; candle is the named fallback — DEFAULT
...
### D-07a — Full-text engine: SQLite FTS5 over the crate's existing `rusqlite` (bundled) dependency, NOT tantivy — PROPOSED (X06.4 worker; orchestrator to confirm)
