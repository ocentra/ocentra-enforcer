...
Use `ocentra-enforcer init --root <repo> --profile <profile> --adapters codex,mcp,precommit,github-actions --dry-run` before writing generated wiring. The default pre-commit adapter is a plain Git hook for cross-platform use; Husky is generated only when requested or when the target repo already uses Husky.
