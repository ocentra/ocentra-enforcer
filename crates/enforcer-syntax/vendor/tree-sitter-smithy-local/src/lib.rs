//! Vendored Smithy tree-sitter grammar binding (language-parity wave
//! G2.4c). WHY VENDORED rather than depending on the published
//! `tree-sitter-smithy` 0.0.1 crate directly: that crate's own
...
//! directory is a standard MIT template, since the published
//! crates.io package ships no `LICENSE` file to copy verbatim.
...
/// Returns the Smithy tree-sitter [`LanguageFn`]. Same calling
/// convention as every other vendored grammar in this workspace.
pub fn language() -> LanguageFn {
    // SAFETY: `tree_sitter_smithy` is the unmodified generated parser
    // entry point from the upstream `tree-sitter-smithy` 0.0.1 crate's
    // own `src/parser.c`, ABI 14.
    unsafe { LanguageFn::from_raw(tree_sitter_smithy) }
}
