...
use language_registry::{ManifestError, render_source, validate_source};
use std::env;
use std::fs;
use std::path::PathBuf;

const REVIEWED_MANIFEST: &str = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/registry/languages.json"));
...
fn main() -> Result<(), ManifestError> {
    ...
    let output_directory = PathBuf::from(env::var_os(OUT_DIR_ENV).ok_or(ManifestError::MissingOutputDirectory)?);
    fs::write(output_directory.join(GENERATED_FILE_NAME), generated).map_err(|error| ManifestError::Io(error.to_string()))?;
    Ok(())
}
