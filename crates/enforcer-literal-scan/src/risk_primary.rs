use crate::risk_heuristics::{is_import_like_context, is_magic_string_comparison, is_secret_like};
...
pub(crate) fn primary_category(candidate: &LiteralCandidate, role: FileRole) -> RiskCategory {
    ...
    if let Some(category) = role_specific_category(role, context, text) {
        return category;
    }
