//! Append-only SHA-256 hash-chained NDJSON journal.
//!
...
//! envelope: a versioned [`JournalRecord`] wire shape (`schema_version` +
...
use enforcer_core::hash_chain::{link_digest, verify_chain, ChainBreak};
...
#[cfg(test)]
mod tests {
    use super::{JournalRecord, ProofJournal, JOURNAL_SCHEMA_VERSION};
    ...
    fn sample_record(event_type: &str, secret: &str) -> JournalRecord {
        JournalRecord {
    ...
    fn tampered_record_fails_closed_on_open() -> Result<()> {
        ...
        assert!(outcome.is_err(), "tampered journal must fail to open");
        std::fs::remove_file(&path)?;
        Ok(())
    }
    ...
    fn tampered_record_fails_closed_on_replay() -> Result<()> {
        ...
        assert!(outcome.is_err(), "tampered journal must fail replay");
        std::fs::remove_file(&path)?;
        Ok(())
    }
