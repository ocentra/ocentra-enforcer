...
impl Validator for TimeValidator {
    ...
    fn validate(&self, input: ValidationInput<'_>) -> Vec<Finding> {
        ...
        let has_skew_tolerance = self.skew_tolerance.is_match(input.source);
...
mod tests {
    ...
    fn mcm_time() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_fixture_parity(
            &validator,
            &manifest_dir(),
            "tests/fixtures/money_critical_mechanics/time/bad/client_clock.ts",
            "tests/fixtures/money_critical_mechanics/time/good/server_clock.ts",
        )?;
        Ok(())
    }
    ...
    fn fires_on_client_now_identifier() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let findings = validator.validate(ValidationInput {
            file: &file,
            source: "const expiry = clientNow + windowMs;\n",
