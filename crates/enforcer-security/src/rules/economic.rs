...
impl Validator for EconomicValidator {
    ...
    fn validate(&self, input: ValidationInput<'_>) -> Vec<Finding> {
        let has_retry = self.retry_construct.is_match(input.source);
        let has_backend_cost = self.backend_cost.is_match(input.source);
        if !has_retry || !has_backend_cost {
            return Vec::new();
        }

        let mut score = 50;
        if self.bound_or_charge.is_match(input.source) {
            score -= 60;
        }
        ...
        vec![Finding {
            ...
            title: "retry loop with non-zero backend cost and no bound/charge (T2 scored)"
                .to_owned(),
            detail: format!(
...
mod tests {
    ...
    fn mcm_economic() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_fixture_parity(
            &validator,
            &manifest_dir(),
            "tests/fixtures/money_critical_mechanics/economic/bad/free_retry.ts",
            "tests/fixtures/money_critical_mechanics/economic/good/bounded_cost.ts",
        )?;
        Ok(())
    }
