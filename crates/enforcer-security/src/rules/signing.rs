...
impl Validator for SigningValidator {
    ...
    fn validate(&self, input: ValidationInput<'_>) -> Vec<Finding> {
        ...
        let has_correlation_log = self.correlation_log.is_match(input.source);
...
mod tests {
    ...
    fn mcm_signing() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_fixture_parity(
            &validator,
            &manifest_dir(),
            "tests/fixtures/money_critical_mechanics/signing/bad/sign_client_raw.ts",
    ...
    fn fires_on_bare_body_identifier_sign_call() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let findings = validator.validate(ValidationInput {
            file: &file,
            source: "logger.info('sign', { correlationId });\nconst sig = sign(body);\n",
