...
pub fn map_to_profile(
    ...
    for asserted in &spec.asserted_rules {
        let backed = catalog.is_backed(&asserted.rule_id);
        ...
        if let Some(finding) = crate::boundary::finding::from_source(
            (&asserted.rule_id, Severity::Warning),
