...
use enforcer_domain::{boundary::decode_error::DecodeError, ids::RuleId};

/// The set of canonical [`RuleId`] values this crate currently has a real, registered
...
impl BackedRuleCatalog {
    ...
    {
        Self {
            backed_ids: ids.into_iter().collect(),
        }
    }

    /// Decode raw rule tokens at the explicit policy/configuration boundary.
    /// Once construction succeeds, the catalog stores only canonical [`RuleId`]
    /// values and no private stringly-typed invariant remains.
    pub fn from_raw_ids<I, S>(ids: I) -> Result<Self, DecodeError>
    where
        I: IntoIterator<Item = S>,
        S: AsRef<str>,
    {
        ids.into_iter()
            .map(|raw| RuleId::from_str(raw.as_ref()))
            .collect::<Result<BTreeSet<_>, _>>()
            .map(|backed_ids| Self { backed_ids })
    }
    ...
    pub fn track_h_snapshot() -> Result<Self, DecodeError> {
        Self::from_raw_ids([
            "H00-1.1",
    ...
    pub fn is_backed(&self, rule_id: &RuleId) -> bool {
