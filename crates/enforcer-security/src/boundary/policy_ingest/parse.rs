...
//! [`PolicyIngestError`].
//! BOUNDARY-INVARIANT: raw policy text either becomes a fully typed policy
//! specification or a source-located error; malformed recognized entries
//! are never silently accepted.
//! PROPERTY-TEST: tests/policy_ingest.rs generates category documents and
//! proves stable first-occurrence normalization.
...
pub fn parse_spec(source_name: &str, source: &str) -> Result<PolicySpec, PolicyIngestError> {
    ...
    for (index, raw_source_line) in source.lines().enumerate() {
        let raw_line = u32::try_from(index).unwrap_or(u32::MAX).saturating_add(1);
        let line_number = SourceLine::try_new(NonZeroU32::new(raw_line).unwrap_or(NonZeroU32::MIN));
        let line = raw_source_line.trim();
        ...
        match section {
            REQUIRED_CATEGORIES_HEADER => {
                let category =
                    SecurityTestCategory::try_from(item.to_owned()).map_err(|error| {
                ...
                push_unique(&mut spec.required_test_categories, category);
            }
            INVARIANTS_HEADER => {
                let invariant =
                    SecurityInvariantId::try_from(item.to_owned()).map_err(|error| {
                        PolicyIngestError::MalformedEntry {
                            spec_source: source_name.to_owned(),
                            section: section.to_owned(),
                            reason: error.reason,
                ...
                push_unique(&mut spec.invariants, invariant);
            }
            RULES_HEADER => {
...
fn parse_rule_line(
    ...
    let rule_id_token = item
        .get(..open)
        .ok_or_else(|| PolicyIngestError::MalformedEntry {
            spec_source: source_name.to_owned(),
            section: section.to_owned(),
            reason: format!("rule entry `{item}` has an invalid rule-id boundary"),
        })?
        .trim();
    let tier_token = item
        ...
        .ok_or_else(|| PolicyIngestError::MalformedEntry {
            spec_source: source_name.to_owned(),
            section: section.to_owned(),
            reason: format!("rule entry `{item}` has an invalid tier boundary"),
        })?
        .trim();
