...
//! BOUNDARY-INVARIANT: untrusted SKILL.md frontmatter is decoded and checked
//! before any vocabulary identifiers enter the validator result.
//! boundaryOwnerNote: this module owns the security-skill frontmatter parser
//! and its transport-to-finding conversion.
//! NEGATIVE-TEST: tests/cyberskills_parity.rs exercises missing, malformed,
//! and unknown-vocabulary frontmatter through the production validator.
//!
...
impl SkillFrontmatterValidValidator {
    /// Construct the validator and compile its canonical syntax patterns.
    ...
    fn structural_errors(&self, fm: &Frontmatter) -> Vec<String> {
        ...
        fm.field("domain")
            .filter(|value| *value != "cybersecurity")
            .into_iter()
            .for_each(|unexpected| {
                errors.push(format!(
                    "domain must be 'cybersecurity', got '{unexpected}'"
                ));
            });
...
impl Validator for SkillFrontmatterValidValidator {
    ...
    fn validate(&self, input: ValidationInput<'_>) -> Vec<Finding> {
        let Some(fm) = parse_frontmatter(input.source.as_str()) else {
            return Vec::new();
        };
...
mod tests {
    ...
    fn cyberskills_frontmatter_lint() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_fixture_parity(
            ...
            &"tests/fixtures/cyberskills/skill_frontmatter/bad/bad_attack_id.md".parse()?,
            &"tests/fixtures/cyberskills/skill_frontmatter/good/valid.md".parse()?,
        )?;
        Ok(())
    }
}
