...
//! BOUNDARY-INVARIANT: untrusted Rust source is parsed with syn before
//! classification signals are evaluated or findings are constructed.
//! NEGATIVE-TEST: tests/money_critical_parity.rs and mechanics parity fixtures
//! cover malformed, unannotated, and non-money-critical source shapes.
//!
...
mod tests {
    ...
    fn money_crit_classify_fixture_parity() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_fixture_parity(
            ...
            &"tests/fixtures/money_critical/classify/bad/credit_balance.rs".parse()?,
            &"tests/fixtures/money_critical/classify/good/pure_formatter.rs".parse()?,
        )?;
        Ok(())
    }
    ...
    fn money_crit_annotated_fixture_parity() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_fixture_parity(
            &validator,
            &enforcer_domain::paths::RepoRoot::try_from(manifest_dir().as_path())?,
            &"tests/fixtures/money_critical/annotated/bad/sign_payment_unannotated.rs".parse()?,
            &"tests/fixtures/money_critical/annotated/good/sign_payment_registered.rs".parse()?,
        )?;
        Ok(())
    }
    ...
    fn classify_annotated_pair_stays_clean_under_annotated_gate(
        ...
        let findings = validator.validate(ValidationInput {
            file: &file,
            source: enforcer_domain::boundary::validation::ValidationSource::from_text("#[money_critical(registered)]\nfn credit_balance(id: u64, amount: u64) { let _ = (id, amount); }\n"),
    ...
    fn ambiguous_unit_still_gates_like_a_confident_classification(
        ...
        let findings = validator.validate(ValidationInput {
            file: &file,
            source: enforcer_domain::boundary::validation::ValidationSource::from_text(
                "fn schedule_payout(id: u64) { let _ = id; }\n",
            ),
            scope: ScanScope::Files,
        });
