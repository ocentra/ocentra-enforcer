...
impl Validator for KillSwitchValidator {
    ...
    fn validate(&self, input: ValidationInput<'_>) -> Vec<Finding> {
        if !self.kill_switch.is_match(input.source.as_str()) {
            return Vec::new();
        }

        let mut missing = Vec::new();
        if !self.atomic.is_match(input.source.as_str()) {
            missing.push("atomic (no withLock/transaction/atomic wrapper)");
        }
        if !self.authed.is_match(input.source.as_str()) {
            missing.push("authenticated (no requireAuth/authenticate/verifyServiceToken)");
        }
        if !self.audited.is_match(input.source.as_str()) {
            missing.push("audited (no auditLog/audit call)");
        }
        if !self.replay_safe.is_match(input.source.as_str()) {
            missing.push("replay-safe (no idempotencyKey/replayGuard)");
        }
        if !self.tested.is_match(input.source.as_str()) {
