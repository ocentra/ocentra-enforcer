...
//! BOUNDARY-INVARIANT: untrusted invariant-suite JSON is decoded before
//! validator decisions and malformed records remain outside domain state.
//! NEGATIVE-TEST: tests/economic_invariants_parity.rs exercises malformed,
//! missing, and wrong-shape property records through the production validator.
//!
...
// BOUNDARY-INVARIANT: this validator decodes external invariant manifests and emits canonical findings.
// boundaryOwnerNote: enforcer-security owns the target-manifest parsing boundary for economic invariant validation.
...
struct PropertyEntry {
    shape: PropertyShape,
    // DEFAULT-JUSTIFICATION: test identifiers are diagnostic context only;
    // absent text does not change whether the property shape is present.
    #[serde(default)]
    test: String,
}
...
struct UnitEntry {
    unit: String,
    // DEFAULT-JUSTIFICATION: a unit with no property map records zero covered
    // invariants and is reported by the validator rather than rejected.
    #[serde(default)]
...
impl Validator for EconomicInvariantPresenceValidator {
    ...
    fn validate(&self, input: ValidationInput<'_>) -> Vec<Finding> {
        let Some(suite) = parse_invariant_suite(input.source.as_str()) else {
            return Vec::new();
        };
...
mod tests {
    ...
    fn presence_fixture_parity_failure_not_reward() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_fixture_parity(
            ...
            &"tests/fixtures/economic_invariants/failure_not_reward/bad/missing_failure_not_reward.json".parse()?,
            &"tests/fixtures/economic_invariants/full_suite/good/full_suite.json".parse()?,
        )?;
    ...
    fn presence_fixture_parity_attacker_cost() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_fixture_parity(
            ...
            &"tests/fixtures/economic_invariants/attacker_cost/bad/cost_no_property.json"
                .parse()?,
            &"tests/fixtures/economic_invariants/attacker_cost/good/cost_property.json".parse()?,
        )?;
        Ok(())
    }
    ...
    fn shape_fixture_parity_idempotency() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_fixture_parity(
            &validator,
            &enforcer_domain::paths::RepoRoot::try_from(manifest_dir().as_path())?,
            &"tests/fixtures/economic_invariants/idempotency/bad/single_case.json".parse()?,
            &"tests/fixtures/economic_invariants/idempotency/good/property.json".parse()?,
        )?;
        ...
        assert_eq!(bad_findings.len(), 1);
        assert!(bad_findings[0].detail.as_str().contains("single_case"));
        Ok(())
    }
