...
//! directives:
//!
//! - Rust: lint-suppression attributes or Clippy allowances on the deny wall.
...
//! BOUNDARY-INVARIANT: raw source lines are interpreted only as candidate
//! suppression directives and are emitted as typed findings.
//! NEGATIVE-TEST: tests/fixtures/no-bypass/good proves ordinary source remains
//! clean while tests/fixtures/no-bypass/bad exercises forbidden shapes.
...
impl NoBypassValidator {
    /// Construct the validator from its fixed suppression-directive patterns.
    pub fn new() -> Result<Self, DecodeError> {
        let specs: &[(&str, &str)] = &[
            ...
            (
                concat!("clippy", "::allow on the deny wall"),
...
mod tests {
    ...
    fn no_bypass_fixture_parity() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_fixture_parity(
            ...
            &"tests/fixtures/no-bypass/bad/sample.rs".parse()?,
            &"tests/fixtures/no-bypass/good/sample.rs".parse()?,
        )?;
        Ok(())
    }
}
