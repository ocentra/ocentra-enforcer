...
fn detects_unauthenticated_internal_route_fixture() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_fixture_parity(
        ...
        &"tests/fixtures/money_critical_mechanics/boundary/bad/unauthed_internal.ts".parse()?,
        &"tests/fixtures/money_critical_mechanics/boundary/good/authed_internal.ts".parse()?,
    )?;
    Ok(())
}
...
fn detects_trusted_internal_header_without_verification() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let findings = validator.validate(ValidationInput {
        ...
        source: enforcer_domain::boundary::validation::ValidationSource::from_text(
            "if (req.headers['x-internal'] === 'true') { grantAccess(); }\n",
        ),
    ...
    assert_eq!(
        ...
        "internal header trusted without verification (T1)"
    );
    Ok(())
}
