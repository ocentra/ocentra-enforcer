...
mod tests {
    ...
    fn write_temp(contents: &[u8]) -> Result<(NamedTempFile, String)> {
    ...
    #[test]
    fn artifact_hash_validation_accepts_exact_hash() -> Result<()> {
    ...
    #[test]
    fn artifact_hash_validation_accepts_uppercase_manifest_hash() -> Result<()> {
        let (artifact, artifact_hash) = write_temp(b"model")?;
        let (tokenizer, tokenizer_hash) = write_temp(b"tokenizer")?;
        let spec = ModelSpec::qwen3_embedding(
            artifact.path(),
    ...
    #[test]
    fn artifact_hash_validation_rejects_mismatch() -> Result<()> {
    ...
    #[test]
    fn output_validation_rejects_wrong_dimension_and_nan() {
        assert!(validate_embedding_output(&[0.0, 1.0], 3).is_err());
        assert!(validate_embedding_output(&[f32::NAN], 1).is_err());
        assert!(validate_embedding_output(&[0.0, 1.0], 2).is_ok());
    }

    #[test]
    fn reranker_score_validation_is_strict() {
        assert!(validate_reranker_scores(&[0.1], 2).is_err());
        assert!(validate_reranker_scores(&[f32::INFINITY], 1).is_err());
        assert!(validate_reranker_scores(&[0.1, 0.2], 2).is_ok());
    }

    #[cfg(feature = "ort-models")]
    #[test]
    fn ort_models_feature_compiles_fixture_validation_path() -> Result<()> {
        ...
        validate_model_artifacts(&spec)?;
        let manifest = ModelRuntimeManifest::from_spec(&spec, "ort", Some(ProviderKind::Cpu));
