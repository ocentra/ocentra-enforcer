...
mod tests {
    ...
    #[test]
    fn exact_id_returns_exact_content() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn wrong_id_is_fail_closed_not_similar() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn malformed_id_is_not_found_not_panic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn traversal_shaped_ids_are_rejected() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn snippet_exact_shares_the_same_fail_closed_contract() -> Result<(), Box<dyn std::error::Error>>
    {
    ...
    fn sample_graph() -> Result<crate::code_graph::CodeGraph, Box<dyn std::error::Error>> {
        ...
        let mut graph = crate::code_graph::CodeGraph::new();
        graph.index_repository(
            dir.path(),
            &[file_path],
            &crate::code_graph::Manifest::default(),
        )?;
        Ok(graph)
    }

    #[test]
    fn export_then_import_reconstructs_identical_node_and_edge_counts(
    ...
    #[test]
    fn artifact_json_has_exactly_the_baseline_field_set() -> Result<(), Box<dyn std::error::Error>>
    {
        let graph = sample_graph()?;
        let snapshot = GraphSnapshot::from_code_graph(&graph);
    ...
    #[test]
    fn artifact_exists_is_false_until_export_then_true() -> Result<(), Box<dyn std::error::Error>> {
