...
// enforcer-rust vendoring note (language-parity wave G2.3b): the upstream
// body of this function declared `keyword_pointer`/`continue_check` as C99
// variable-length arrays (VLA) sized by the `number_of_words` PARAMETER --
// valid under GCC/Clang but rejected by MSVC's `cl.exe` ("error C2057:
// expected constant expression"), confirmed via a real `cargo build` on
// this workspace's Windows/MSVC toolchain. The one real call site
// (`tree_sitter_COBOL_external_scanner_scan` below) always passes the
// compile-time constant `number_of_comment_entry_keywords` (9), so bounding
// both arrays to a small fixed size is behavior-preserving, not a
// functional change -- COMMENT_ENTRY_KEYWORD_MAX_LEN is set well above that
// with headroom for any future keyword added to `any_content_keyword`.
#define COMMENT_ENTRY_KEYWORD_MAX_LEN 32
...
static bool start_with_word( TSLexer *lexer, char *words[], int number_of_words) {
    ...
    char *keyword_pointer[COMMENT_ENTRY_KEYWORD_MAX_LEN];
    bool continue_check[COMMENT_ENTRY_KEYWORD_MAX_LEN];
