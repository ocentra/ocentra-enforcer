...
> for repo `{{project_name}}`. Read this file after the global tier and
> before any per-plan tier. Budget: stay under {{budget_lines}} lines /
> {{budget_bytes}} bytes for this tier.
<!-- /agents-read-first -->
...
> DECISION TREE
> - if resuming work -> read {{next_tier_path}}
> - if starting fresh -> read {{next_tier_path}}
> - if blocked -> read {{next_tier_path}}
> LEAF -> {{resume_anchor}}
<!-- /agents-decision-tree -->
...
Repo root router for `{{project_name}}`.
