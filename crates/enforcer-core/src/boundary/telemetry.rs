...
use enforcer_domain::boundary::core::{chain_entry_count, chain_link_index};
    ...
    T: serde::Serialize + serde::de::DeserializeOwned,
{
    ...
    pub fn append(&mut self, record: &T) -> Result<Sha256> {
        ...
        let digest = link_digest(self.prev_digest.as_ref(), line.as_bytes());

        self.data_file.write_all(line.as_bytes())?;
        self.data_file.write_all(b"\n")?;
        self.data_file.flush()?;

        self.chain_sidecar.write_all(digest.as_str().as_bytes())?;
    ...
    pub fn last_digest(&self) -> Option<&Sha256> {
...
pub fn verify_file_chain(path: &Path) -> Result<std::result::Result<usize, ChainBreak>> {
    ...
    let recorded_digests: Vec<Sha256> = sidecar_content
        .lines()
        .filter(|l| !l.trim().is_empty())
        .map(str::parse)
    ...
    if recorded_digests.len() != lines.len() {
        return Ok(Err(ChainBreak::length_mismatch(
            chain_link_index(recorded_digests.len().min(lines.len())),
            chain_entry_count(recorded_digests.len()),
            chain_entry_count(lines.len()),
        )));
    }

    let links = lines.iter().copied().zip(recorded_digests.iter());
...
mod tests {
    ...
    fn append_redacts_before_writing() -> Result<()> {
        ...
        let persisted: SampleRecord = serde_json::from_str(raw.trim())?;
        assert_eq!(persisted.secret, "[REDACTED]");
        assert_eq!(persisted.note, "clean");
        cleanup(&path)?;
        Ok(())
    }
