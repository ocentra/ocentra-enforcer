...
pub fn iso8601_utc(epoch_ms: u128) -> String {
    let total_secs = i64::try_from(epoch_ms / 1000).unwrap_or(i64::MAX);
    let millis = u32::try_from(epoch_ms % 1000).unwrap_or_default();
...
fn civil_from_days(z: i64) -> (i64, u32, u32) {
    ...
    let mp = (5 * doy + 2) / 153;
    let day = u32::try_from(doy - (153 * mp + 2) / 5 + 1).unwrap_or_default();
...
mod tests {
    ...
    proptest! {
        #[test]
        fn normalized_separators_are_idempotent_and_backslash_free(path in ".*") {
            let normalized = normalize_separators(&path);
            prop_assert_eq!(normalized.matches('\\').count(), 0);
            prop_assert_eq!(normalize_separators(&normalized), normalized);
        }
    }
