...
//! `rules/rules.json` entry with `language == "iac"` must have a
//! registered [`enforcer_lang_iac::rules::registry`] row (no orphan rule
...
//! 8 — the authoritative IaC rule count this workpack introduces. The test
//! FAILS if `rules/rules.json` gains/loses an iac rule without a matching
...
/// repo-root `rules/rules.json`. `CARGO_MANIFEST_DIR` is
/// `<repo>/crates/enforcer-lang-iac`, so the catalog is two levels up.
const RULES_JSON: &str = include_str!("../../../rules/rules.json");

fn iac_rule_ids_from_catalog() -> Result<BTreeSet<String>, Box<dyn std::error::Error>> {
    ...
    for rule in rules {
        ...
        if language != Some("iac") {
            continue;
        }
        let id = rule
            .get("id")
            .and_then(serde_json::Value::as_str)
            .ok_or("rules/rules.json: iac rule missing `id`")?;
...
#[test]
fn registry_covers_every_iac_rule_id_with_no_orphans_and_no_duplicates(
) -> Result<(), Box<dyn std::error::Error>> {
    let catalog_ids = iac_rule_ids_from_catalog()?;
    assert_eq!(
        catalog_ids.len(),
        8,
        "rules/rules.json iac count drifted from the workpack's authoritative 8"
    );
    ...
    assert!(
        ...
        "rules/rules.json iac rule id(s) with no registered validator: {orphan_catalog_ids:?}"
    );

    let unknown_registry_ids: Vec<_> = registry_ids.difference(&catalog_ids).collect();
    assert!(
        unknown_registry_ids.is_empty(),
        "registry rule id(s) not present in rules/rules.json's iac set: {unknown_registry_ids:?}"
    );

    assert_eq!(
        registry_ids.len(),
        8,
        "registry total must equal the workpack's authoritative iac count (8)"
    );
