...
fn style_parsers_reject_malformed_boundaries_without_panicking(
    ...
    assert!(validate(&signature, "public function save(")?.is_empty());
    assert!(validate(&missing_scope, "function demo() {\nname =")?.is_empty());
    assert_eq!(validate(&signature, "public function save(id)")?.len(), 1);
    assert_eq!(
        validate(&missing_scope, "function demo() {\nname = value;")?.len(),
        1
    );
    Ok(())
}
