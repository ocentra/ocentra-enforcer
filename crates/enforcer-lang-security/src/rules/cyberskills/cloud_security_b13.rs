//! `CYBER-CLOUD-MANIFEST.13` — supplied-artifact cloud capability.
//!
//! BOUNDARY-INVARIANT: the validator checks only caller-supplied offline JSON
//! for IAM, Lambda, Azure Defender, registry, and Kubernetes records. Provider
//! APIs, accounts, credentials, clusters, registries, scanners, networks, and
//! production outcomes remain outside it.
//!
    ...
    Predicate(
        manifest.schema_version == 1
            ...
            && manifest
                .records
                .iter()
                .all(|record| kinds.insert(record.kind.as_str()) && valid_record(record).0),
    )
}
...
impl Validator for CloudSecurityManifestB13Validator {
    ...
    fn validate(&self, input: ValidationInput<'_>) -> Vec<Finding> {
        ...
        crate::boundary::finding::from_source(
            ...
            "The supplied manifest is malformed or missing a required typed IAM, Lambda, Azure Defender, registry, or Kubernetes reference. This is a static schema finding only; no provider, account, credential, cluster, registry, scanner, network, runtime, or security outcome was evaluated.",
            input.file,
