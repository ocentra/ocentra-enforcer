//! BOUNDARY-INVARIANT: CP09 cloud-security B06 tests supplied JSON only.
...
use enforcer_lang_security::rules::cyberskills::cloud_security_b06::CloudSecurityManifestB06Validator;
...
fn fixture(
    ...
    let file: RelPath = "cloud-security-manifest-b06.json".parse()?;
    let validator = CloudSecurityManifestB06Validator::new()?;
...
#[test]
fn cloud_security_manifest_b06_pass_and_boundary_are_clean(
) -> Result<(), Box<dyn std::error::Error>> {
    let pass = fixture("tests/fixtures/cyberskills/cloud-security-manifest-b06/pass.json")?;
    let boundary = fixture("tests/fixtures/cyberskills/cloud-security-manifest-b06/boundary.json")?;
...
#[test]
fn cloud_security_manifest_b06_fail_and_malformed_are_rejected(
) -> Result<(), Box<dyn std::error::Error>> {
    let failures = fixture("tests/fixtures/cyberskills/cloud-security-manifest-b06/fail.json")?;
    let malformed =
        fixture("tests/fixtures/cyberskills/cloud-security-manifest-b06/malformed.json")?;
    assert_eq!(failures.len(), 1);
    assert_eq!(malformed.len(), 1);
    assert!(failures
        .iter()
        .chain(malformed.iter())
        .all(|finding| finding.rule_id.as_str() == "CYBER-CLOUD-MANIFEST.6"));
    Ok(())
}

#[test]
fn cloud_security_manifest_b06_pass_covers_all_five_skills(
) -> Result<(), Box<dyn std::error::Error>> {
    let source = std::fs::read_to_string(
        manifest_dir().join("tests/fixtures/cyberskills/cloud-security-manifest-b06/pass.json"),
    )?;
...
#[test]
fn cloud_security_manifest_b06_preserves_static_only_boundary(
) -> Result<(), Box<dyn std::error::Error>> {
    let source = std::fs::read_to_string(
        manifest_dir().join("tests/fixtures/cyberskills/cloud-security-manifest-b06/boundary.json"),
    )?;
    let document: serde_json::Value = serde_json::from_str(&source)?;
