...
type WorkpackIndexPayload = { sourcePath: string; rows: WorkpackIndexRow[]; statusCounts: Record<string, number>; caveat: string };
...
function formatTarget(target: EngineCapabilityTarget): string {
  ...
  const subviewLabels: Record<string, string> = { harnesses: "Adapters", lanes: "Lane Hub" };
  const workspaceLabels: Partial<Record<WorkspaceKey, string>> = { projects: "Projects", findings: "Scan", doctrine: "Policy", proofs: "Proofs", memory: "Memory", engine: "Engine" };
...
export function EngineWorkspace({ capabilities, loading, error, workpackIndex, workpackIndexLoading, workpackIndexError, onNavigate }: { capabilities?: EngineCapability[]; loading: boolean; error: string; workpackIndex?: WorkpackIndexPayload; workpackIndexLoading: boolean; workpackIndexError: string; onNavigate: (target: EngineCapabilityTarget) => void }) {
  ...
  const [focusedWorkpackIds, setFocusedWorkpackIds] = useState<string[]>();
  ...
  return <section className="main-surface engine-workspace">
    ...
    <div className="engine-toolbar"><ListFilter size={16} /><div className="segmented-control" aria-label="Engine view">{(["capabilities", "workpacks"] as EngineView[]).map((view) => <button key={view} className={engineView === view ? "active" : ""} onClick={() => { if (view === "workpacks") setFocusedWorkpackIds(undefined); setEngineView(view); }}>{view === "capabilities" ? "Capabilities" : "Workpacks"}</button>)}</div><small>{engineView === "capabilities" ? "Rust-provided product metadata" : "Rust read model of the authored plan index"}</small></div>
...
function CapabilityView({ capabilities, error, filter, filtered, loading, selected, onFilter, onOpenSurface, onOpenWorkpack, onSelect }: { capabilities?: EngineCapability[]; error: string; filter: CapabilityFilter; filtered: EngineCapability[]; loading: boolean; selected?: EngineCapability; onFilter: (filter: CapabilityFilter) => void; onOpenSurface: (capability: EngineCapability) => void; onOpenWorkpack: (id: string) => void; onSelect: (id: string) => void }) {
...
function WorkpackView({ capabilities, error, focusedWorkpackIds, index, linkedCapabilities, loading, mappedWorkpackIds, placement, placementCounts, query, selected, status, statuses, track, tracks, visible, onClearFocus, onOpenSurface, onPlacement, onQuery, onSelect, onStatus, onTrack }: { capabilities?: EngineCapability[]; error: string; focusedWorkpackIds?: string[]; index?: WorkpackIndexPayload; linkedCapabilities: EngineCapability[]; loading: boolean; mappedWorkpackIds: Set<string>; placement: WorkpackPlacementFilter; placementCounts: { mapped: number; unmapped: number }; query: string; selected?: WorkpackIndexRow; status: string; statuses: string[]; track: string; tracks: string[]; visible: WorkpackIndexRow[]; onClearFocus: () => void; onOpenSurface: (capability: EngineCapability) => void; onPlacement: (placement: WorkpackPlacementFilter) => void; onQuery: (query: string) => void; onSelect: (id: string) => void; onStatus: (status: string) => void; onTrack: (track: string) => void }) {
  ...
  return <>
    ...
    <div className="workpack-controls" aria-label="Workpack filters">
      ...
      <label className="workpack-filter"><span>Capability mapping</span><select value={placement} onChange={(event) => onPlacement(event.target.value as WorkpackPlacementFilter)} aria-label="Workpack capability mapping"><option value="all">All mappings</option><option value="mapped">Mapped</option><option value="unmapped">Not mapped</option></select></label>
