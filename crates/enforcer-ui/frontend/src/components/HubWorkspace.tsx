...
export function HubWorkspace({ hub, loading, error, handoff, initialView, harnessDiscovery, harnessDiscoveryLoading, harnessDiscoveryError, onRefreshHarnesses, onSendMessage, onAcknowledgeMessage, onCreateClaim, onClearHandoff }: { hub?: HubPayload | undefined; loading: boolean; error: string; handoff?: HubFindingHandoff | undefined; initialView: HubView; harnessDiscovery?: HarnessDiscoveryPayload | undefined; harnessDiscoveryLoading: boolean; harnessDiscoveryError: string; onRefreshHarnesses: () => Promise<void>; onSendMessage: (recipientLane: string, body: string) => Promise<void>; onAcknowledgeMessage: (messageId: string) => Promise<void>; onCreateClaim: (request: { projectRoot: string; laneId: string; path: string; reason: string }) => Promise<void>; onClearHandoff: () => void }): ReactElement {
  ...
  return (
    <section className="main-surface hub-command-center">
      <div className="scan-panel hub-main-panel">
        ...
        {view === "harnesses" && <HarnessList {...(harnessDiscovery === undefined ? {} : { discovery: harnessDiscovery })} loading={harnessDiscoveryLoading} error={harnessDiscoveryError} query={query} />}
...
function InboxDetail({ message, lanes, onSendMessage, onAcknowledgeMessage }: { message?: HubPayload["mail"][number] | undefined; lanes: HubPayload["lanes"]; onSendMessage: (recipientLane: string, body: string) => Promise<void>; onAcknowledgeMessage: (messageId: string) => Promise<void> }) {
