...
//! fallback.
//! BOUNDARY-INVARIANT: raw host, token, socket, and HTML values are accepted
//! and emitted only here; remote binds fail closed before a listener opens.
//! NEGATIVE-TEST: remote-without-token and empty-token tests reject unsafe binds.
//! boundaryOwnerNote: enforcer-ui owns bind-request parsing and serve transport.
//! The `try_new` path calls `parse` for raw boundary fields before transport
//! decisions.
//!
...
/// Bind options this surface resolves before ever touching a socket.
/// `host`/`port` are the requested bind target; `token` is the
/// caller-supplied auth token (required for non-loopback).
#[derive(Debug, Clone)]
pub struct BindOptions {
    pub host: UiBindHost,
    pub port: u16,
    pub token: Option<UiAuthToken>,
}

impl BindOptions {
    /// Parse raw CLI/MCP bind fields into validated domain values before the
    /// transport gate or listener can observe them.
    pub fn try_new(
        host: String,
        port: u16,
        token: Option<String>,
        ...
        Ok(Self {
            host: UiBindHost::try_from(host)?,
            token: token.map(UiAuthToken::try_from).transpose()?,
            port,
        })
    }
}
...
impl<'a> From<&'a BindOptions> for BindTarget<'a> {
    fn from(request: &'a BindOptions) -> Self {
        Self {
            host: request.host.as_str(),
            token: request.token.as_ref().map(UiAuthToken::as_str),
        }
    }
}

/// Why [`BindOptions`] were refused.
...
pub fn resolve_bind(request: &BindOptions) -> Result<(), BindError> {
    let target = BindTarget::from(request);
...
/// yet (see module docs).
#[must_use]
pub fn ui_tool_response(request: &BindOptions) -> serde_json::Value {
    match resolve_bind(request) {
        Ok(()) => serde_json::json!({
            "ok": true,
            "url": format!("http://{}:{}", request.host.as_str(), request.port),
...
mod tests {
    use super::{
        ...
        ui_tool_response, BindError, BindOptions, ServeAlias, VIEW_MOUNTS,
    };

    fn options(
        host: &str,
        token: Option<&str>,
    ) -> Result<BindOptions, enforcer_domain::boundary::decode_error::DecodeError> {
        BindOptions::try_new(host.to_owned(), 0, token.map(str::to_owned))
    }
    ...
    fn serve_surface_contract_loopback_default_holds() {
        let request = BindOptions::default();
        assert_eq!(request.host.as_str(), "127.0.0.1");
        assert!(request.token.is_none());
        assert_eq!(resolve_bind(&request), Ok(()));
    }
    ...
    #[test]
    fn serve_surface_contract_remote_with_token_resolves() -> Result<(), Box<dyn std::error::Error>>
    {
        let request = options("0.0.0.0", Some("secret"))?;
        assert_eq!(resolve_bind(&request), Ok(()));
        Ok(())
    }
    ...
    fn ui_tool_response_remote_without_token_reports_refusal_without_launching(
        ...
        let error = response["error"]
            .as_str()
            .ok_or("refused response omitted error text")?;
        assert_eq!(error.matches("token").count(), 2);
        Ok(())
    }
}
