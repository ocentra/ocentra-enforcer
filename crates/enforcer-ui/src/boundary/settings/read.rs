...
//! BOUNDARY-INVARIANT: parse external config through `load_project_tie` into
//! `ResolvedProjectTie` before projecting read-only UI rows.
//! boundaryOwnerNote: enforcer-ui owns the typed settings read boundary.
//!
//! camelCase wire casing (locked decision, matches `enforcer-domain`).
//! [`SettingsView`] derives `ts_rs::TS` so [`crate::ts_export`]
...
use enforcer_domain::config_types::{EnforcerScope, NativeMode, NativeTool};

/// One rendered per-rule toggle row: enabled/disabled, effective severity
/// override (when present), and the waiver record (owner + reason + ruleId) when
...
/// into the UI's [`SettingsView`]. Total mapping: every recognized
...
#[must_use]
pub fn render_settings_view(source_path: &str, resolved: &ResolvedProjectTie) -> SettingsView {
    ...
    let rule_toggles = resolved
        ...
        .map(|(rule_id, toggle)| RuleToggleRow {
            ...
            enabled: toggle.enabled == enforcer_domain::config_types::RuleEnabled::Enabled,
            severity: toggle.severity.and_then(|severity| {
                serde_json::to_value(severity)
                    .ok()
                    .and_then(|v| v.as_str().map(str::to_owned))
            }),
            waiver_owner: toggle.waiver.as_ref().map(|w| w.owner.as_str().to_owned()),
            waiver_reason: toggle.waiver.as_ref().map(|w| w.reason.as_str().to_owned()),
        })
        .collect();
...
/// into a [`SettingsView`]. The one entry point this module's GET
...
pub fn load_settings_view(
    config_path: &Path,
) -> enforcer_config::error::ConfigResult<SettingsView> {
...
mod tests {
    ...
    fn settings_read_renders_effective_config() -> Result<(), Box<dyn std::error::Error>> {
        ...
        .to_string();
        let raw = ConfigJson::from_owned(raw);
        let source = ConfigSource::from_owned("cfg.json".to_owned());
        let resolved = parse_project_tie(&raw, &source)?;
    ...
    /// than erroring or hardcoding a fabricated row — matches arc-03's
    ...
    fn settings_read_renders_zero_config_default() -> Result<(), Box<dyn std::error::Error>> {
        let default_resolved = enforcer_config::project_tie::ResolvedProjectTie::resolve(
            ...
            &ConfigSource::from_owned("<none>".to_owned()),
        )?;
    ...
    fn settings_read_fails_closed_on_malformed_config() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let Err(error) = load_settings_view(&path) else {
            return Err("an unknown native-tool key must fail the settings read closed".into());
        };
