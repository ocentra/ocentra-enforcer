...
//! BOUNDARY-INVARIANT: all interpolated catalog text is escaped before HTML output.
//! NEGATIVE-TEST: incomplete records and hostile markup are rejected or escaped.
//! boundaryOwnerNote: enforcer-ui owns the offline explorer HTML projection.
//!
...
//! typed [`super::ExplorerResponse`] into ONE self-contained HTML document:
...
use super::{
    render_explorer, CompletenessFlag, ExplorerResponse, RuleEntryResponse, SkillEntryResponse,
};
...
#[must_use]
pub fn render_rule_card(entry: &RuleEntryResponse) -> String {
...
#[must_use]
pub fn render_skill_card(skill: &SkillEntryResponse) -> String {
    let mut card = String::new();
...
#[must_use]
pub fn render_explorer_html(payload: &ExplorerResponse) -> String {
...
pub fn render_explorer_view(
    ...
    if matches!(mode, UiRunMode::Silent) {
        ...
        render_explorer_html(&render_explorer(UiRunMode::Human, registry, skills_root))
    }
}
...
mod tests {
    ...
    use crate::explorer::render_rule;
    ...
    fn record(
        ...
        Ok(RuleRecord {
            ...
            title: rule_text::<RuleTitle>("No re-export barrels")?,
            tier,
            ...
            tags: vec![
                rule_text::<RuleTag>("rust")?,
                rule_text::<RuleTag>("reexports")?,
            ],
    ...
    fn rendered_html_fetches_no_external_asset() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let html =
            render_explorer_view(UiRunMode::Human, &registry, std::path::Path::new("skills"));
        for forbidden in [
    ...
    fn t3_rule_card_shows_doctrine_not_hard() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(card.matches("class=\"badge doctrine\"").count(), 1);
        Ok(())
    }
    ...
    #[test]
    fn incomplete_rule_card_is_rejected_before_presentation(
    ) -> Result<(), Box<dyn std::error::Error>> {
        assert_eq!(
            RuleDocAnchor::try_from("   ".to_owned()).map(|_| ()),
    ...
    fn interpolated_markup_is_escaped() -> Result<(), Box<dyn std::error::Error>> {
        ...
        evil.title = rule_text::<RuleTitle>("<script src=https://evil/x.js></script>")?;
        let entry = render_rule(&evil);
        let card = render_rule_card(&entry);
        assert_eq!(card.matches("<script src=https://evil").count(), 0);
        assert_eq!(card.matches("&lt;script").count(), 1);
        Ok(())
    }
    ...
    fn silent_mode_renders_no_html() -> Result<(), Box<dyn std::error::Error>> {
        let registry = RuleRegistry::from_records(vec![record("RR-1.1", Tier::T1)?])?;
        let html =
            render_explorer_view(UiRunMode::Silent, &registry, std::path::Path::new("skills"));
    ...
    fn empty_payload_renders_empty_state() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let html =
            render_explorer_view(UiRunMode::Human, &registry, std::path::Path::new("skills"));
        assert_eq!(html.matches("No rules in the registry.").count(), 1);
        assert!(html.starts_with("<!doctype html>"));
        Ok(())
    }
    ...
    fn full_page_has_both_sections() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(html.matches("data-view=\"explorer-rules\"").count(), 1);
        assert_eq!(html.matches("data-view=\"explorer-skills\"").count(), 1);
        Ok(())
    }
}
