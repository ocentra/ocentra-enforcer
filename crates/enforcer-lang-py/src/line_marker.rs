//! The reusable line-scan [`Validator`] every PY source/test-shape rule in
        ...
        for (idx, line) in input.source.lines().enumerate() {
            if let Some(marker) = matches_line(line, self.guard, self.markers) {
                findings.push(Finding {
                    ...
                    detail: format!("matched marker `{marker}`"),
    ...
    fn validate(&self, input: ValidationInput<'_>) -> Vec<Finding> {
        ...
        for (idx, line) in input.source.lines().enumerate() {
            ...
            findings.push(Finding {
                ...
                severity: Severity::Error,
                title: self.title.to_owned(),
                detail: "required companion argument/keyword is missing on this line".to_owned(),
                file: input.file.clone(),
                line: (idx as u32) + 1,
                snippet: Some(line.trim().to_owned()),
            });
...
impl WeakAssertionValidator {
    ...
    pub fn new(rule_id: RuleId, title: &'static str, identifiers: &'static [&'static str]) -> Self {
        Self {
            rule_id,
            title,
            identifiers,
    ...
    fn validate(&self, input: ValidationInput<'_>) -> Vec<Finding> {
        ...
        for (idx, line) in input.source.lines().enumerate() {
            ...
            findings.push(Finding {
                ...
                severity: Severity::Error,
                title: self.title.to_owned(),
                detail: format!("bare truthiness assertion `assert {rest}`"),
                file: input.file.clone(),
                line: (idx as u32) + 1,
                snippet: Some(trimmed.to_owned()),
            });
        }
        findings
    }
...
fn contains_at_word_boundary(haystack: &str, needle: &str) -> bool {
    ...
    while let Some(rel_idx) = haystack[start..].find(needle) {
        let idx = start + rel_idx;
        let boundary_ok = idx == 0 || !is_identifier_byte(bytes[idx - 1]);
        if boundary_ok {
            return true;
        }
        start = idx + 1;
...
mod tests {
    ...
    fn trailing_comment_guard_ignores_prose_mid_statement() {
        ...
        assert!(matches_line("x = 1  # noqa", Guard::TrailingComment, markers).is_some());
        assert!(matches_line(
    ...
    fn not_in_comment_or_string_guard_ignores_comment_mentions() {
        ...
        assert!(matches_line("except Exception:", Guard::NotInCommentOrString, markers).is_some());
        assert!(matches_line(
    ...
    fn line_starts_with_guard_requires_leading_position() {
        ...
        assert!(matches_line("import *", Guard::LineStartsWith, markers).is_some());
