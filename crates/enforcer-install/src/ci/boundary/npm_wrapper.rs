...
//! BOUNDARY-INVARIANT: npm wrapper input is decoded before typed rendering.
//! Negative invalid inputs are rejected during wrapper decoding.
//!
...
#[cfg(test)]
mod tests {
    use enforcer_domain::install_types::Libc;
    ...
    use enforcer_domain::install_types::TargetPlatform;
    ...
    fn rendered_wrapper_package_json_contains_no_hardcoded_local_absolute_path() {
        let json = render_wrapper_package_json("0.1.0");
        assert!(!json.as_str().contains("E:/"));
        assert!(!json.as_str().contains("C:/Projects"));
        assert!(!json.as_str().contains("/home/"));
    }
}
