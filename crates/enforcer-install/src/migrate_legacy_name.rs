...
//! reports zero [`Finding`]s and writes zero bytes on a second run. The
//! single one-time notice ([`MigrationOutcome::notice`]) is emitted exactly
...
pub enum ConfigFormat {
    /// A JSON harness config with a top-level `mcpServers` map (Claude
    /// Code's `~/.claude.json` shape).
    JsonMcpServers,
    /// A TOML harness config with a top-level `mcp_servers` table
    /// (Codex's `config.toml` shape).
    TomlMcpServers,
    /// A free-text managed-doctrine file (e.g. Codex's `AGENTS.md`)
    /// that may embed the legacy server name / tool-name-family prefixes
    /// in prose rather than a structured registry key.
    ManagedText,
}
...
impl ConfigTarget {
    ...
    #[must_use]
    pub fn new(harness: impl Into<String>, path: impl Into<PathBuf>, format: ConfigFormat) -> Self {
        Self {
...
// ---------------------------------------------------------------------
// Structured report types — c01/arc-23 Finding/ApplyResult shape
// ---------------------------------------------------------------------

/// What kind of legacy residue a [`Finding`] describes.
...
pub enum FindingKind {
    ...
    /// The retired `skills/rust-rules-hard-gate/` legacy skill source
    /// tree (or its `agents/openai.yaml` descriptor) still present on
    /// disk.
...
/// `Report`/`ApplyResult` shape this workpack's requirement checklist
...
/// Read-only detection: report every [`Finding`] across `targets` without
...
pub fn scan(targets: &[ConfigTarget]) -> InstallResult<Vec<Finding>> {
...
fn scan_one(target: &ConfigTarget) -> InstallResult<Vec<Finding>> {
    if !target.path.is_file() {
        return Ok(Vec::new());
    }
    let raw = std::fs::read_to_string(&target.path).map_err(|e| io_err(&target.path, e))?;

    match target.format {
        ConfigFormat::JsonMcpServers => scan_json(target, &raw),
        ConfigFormat::TomlMcpServers => scan_toml(target, &raw),
        ConfigFormat::ManagedText => Ok(scan_text_literals(target, &raw)),
    }
}
...
fn scan_json(target: &ConfigTarget, raw: &str) -> InstallResult<Vec<Finding>> {
    let value: JsonValue =
...
fn scan_toml(target: &ConfigTarget, raw: &str) -> InstallResult<Vec<Finding>> {
...
/// JSON/TOML).
fn scan_text_literals(target: &ConfigTarget, raw: &str) -> Vec<Finding> {
    let mut findings = Vec::new();
    if raw.contains(LEGACY_CANONICAL_TOOL_PREFIX) {
        findings.push(Finding {
            harness: target.harness.clone(),
            path: target.path.display().to_string(),
            kind: FindingKind::LegacyToolNameLiteral,
            detail: format!("contains `{LEGACY_CANONICAL_TOOL_PREFIX}*` tool-name literal"),
        });
    }
    if raw.contains(LEGACY_ALIAS_TOOL_PREFIX) {
        findings.push(Finding {
            ...
            path: target.path.display().to_string(),
            kind: FindingKind::LegacyToolNameLiteral,
            detail: format!("contains `{LEGACY_ALIAS_TOOL_PREFIX}*` tool-name literal"),
        });
    }
    if target.format == ConfigFormat::ManagedText && raw.contains(LEGACY_SERVER_NAME) {
        findings.push(Finding {
            ...
            path: target.path.display().to_string(),
            kind: FindingKind::LegacyServerRegistration,
            detail: format!("prose reference to `{LEGACY_SERVER_NAME}`"),
        });
    }
    findings
}
...
/// repo/install root a caller resolves): detect every [`Finding`], rewrite
...
/// a [`MigrationOutcome`] carrying the full typed diff. A second call with
/// the same `targets`/`legacy_skill_dir` after a successful migration is a
/// no-op ([`MigrationOutcome::is_noop`] `true`) that writes zero bytes.
///
/// # Errors
    ...
    if let Some(dir) = legacy_skill_dir {
        if dir.exists() {
            outcome.findings.push(Finding {
                ...
                detail: "legacy skills/rust-rules-hard-gate dir present".to_owned(),
            });
            std::fs::remove_dir_all(dir).map_err(|e| io_err(dir, e))?;
            outcome.retired_skill_dir = Some(dir.display().to_string());
        }
    }

    if !outcome.is_noop() {
        outcome.notice = Some(MIGRATION_NOTICE.to_owned());
    }
...
fn rewrite_target(target: &ConfigTarget) -> InstallResult<()> {
    ...
    std::fs::write(&target.path, rewritten).map_err(|e| io_err(&target.path, e))
}
...
mod tests {
    use super::{
        migrate, scan, ConfigFormat, ConfigTarget, FindingKind, MigrationOutcome,
    ...
    fn claude_target(dir: &std::path::Path) -> ConfigTarget {
        ConfigTarget::new(
            "claude",
    ...
    fn codex_targets(dir: &std::path::Path) -> Vec<ConfigTarget> {
        vec![
            ConfigTarget::new(
                "codex",
            ...
            ConfigTarget::new("codex", dir.join("AGENTS.md"), ConfigFormat::ManagedText),
        ]
    }
    ...
    fn fail_fixture_unmigrated_config_still_contains_legacy_key_verbatim(
        ...
        assert!(raw.contains(LEGACY_SERVER_NAME));
        Ok(())
    }
    ...
    fn pass_fixture_claude_migrate_rewrites_legacy_entry_to_enforcer(
        ...
        assert!(
            !raw.contains(LEGACY_SERVER_NAME),
    ...
    fn pass_fixture_codex_migrate_rewrites_toml_and_agents_md(
        ...
        assert!(!toml_raw.contains(LEGACY_SERVER_NAME));
        assert!(toml_raw.contains(&format!("[mcp_servers.{SERVER_NAME}]")));
        assert!(toml_raw.contains("[mcp_servers.other-tool]"));
        assert!(toml_raw.contains("[misc]"));

        let agents_raw = fs::read_to_string(dir.path().join("AGENTS.md"))?;
        assert!(!agents_raw.contains(LEGACY_SERVER_NAME));
        assert!(!agents_raw.contains(LEGACY_CANONICAL_TOOL_PREFIX));
        assert!(!agents_raw.contains(LEGACY_ALIAS_TOOL_PREFIX));
        assert!(agents_raw.contains(&format!("mcp__{SERVER_NAME}__check")));
        assert!(agents_raw.contains(&format!("mcp__{SERVER_NAME}__run")));
        assert!(agents_raw.contains("# Some User Content"));
        assert!(agents_raw.contains("# Other user content stays untouched"));
    ...
    #[test]
    fn migration_outcome_round_trips_through_json() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let back: MigrationOutcome = serde_json::from_str(&wire)?;
