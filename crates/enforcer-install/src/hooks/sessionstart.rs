...
pub enum HookEvent {
    /// Fires once per new Claude Code session, before the first turn.
    SessionStart,
}
...
/// Compute the [`SessionStartHookConfig`] record the c03 adapter registers.
...
#[must_use]
pub fn sessionstart_hook_config(enforcer_binary_path: &Path) -> SessionStartHookConfig {
...
mod tests {
    ...
    fn reminder_body_contains_enforcer_first_marker() {
        ...
        assert!(body.starts_with(ENFORCER_FIRST_MARKER));
        assert!(body.contains(ENFORCER_FIRST_MARKER));
    }

    #[test]
    fn reminder_body_names_the_concrete_mcp_tools_and_deny_gate() {
        let body = reminder_body(&sample_binary_path());
        assert!(body.contains("enforcer scan"));
        assert!(body.contains("enforcer check"));
        assert!(body.contains("mcp__enforcer__ocentra_enforcer_scan"));
        assert!(body.contains("mcp__enforcer__ocentra_enforcer_check"));
        assert!(body.contains("mcp__enforcer__ocentra_enforcer_coordination_guard"));
        assert!(body.contains("PreToolUse deny gate"));
    }

    #[test]
    fn reminder_body_carries_every_doctrine_tier_token() {
        let body = reminder_body(&sample_binary_path());
        assert!(body.contains(TIER_T1_TOKEN));
        assert!(body.contains(TIER_T2_TOKEN));
        assert!(body.contains(TIER_T3_TOKEN));
    }
    ...
    #[test]
    fn hook_config_round_trips_through_json() -> Result<(), Box<dyn std::error::Error>> {
        let config = sessionstart_hook_config(&sample_binary_path());
        let wire = serde_json::to_string(&config)?;
        assert!(wire.contains("\"sessionStart\""));
        assert!(wire.contains("\"reminderBody\""));
