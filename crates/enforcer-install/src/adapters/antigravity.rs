...
const HARNESS_KEY: &str = "antigravity";
...
mod tests {
    ...
    fn harness_key_is_antigravity() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let adapter = AntigravityAdapter::new(home.path(), home.path().join("enforcer"));
        assert_eq!(adapter.harness_key(), "antigravity");
        Ok(())
    }
    ...
    fn fresh_install_apply_then_verify_all_green() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let adapter = AntigravityAdapter::new(home.path(), &binary);
