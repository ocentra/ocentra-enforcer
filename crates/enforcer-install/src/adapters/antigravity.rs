//! c09 — the Antigravity CLI [`crate::core::HarnessAdapter`].
//!
//! # Charter (workpack c09, reference-multiharness-install-matrix —
//! BINDING)
//!
//! Antigravity is detected via `~/.gemini/antigravity-cli` (c02 autodetect
//! probes `~/.antigravity`/`ANTIGRAVITY_HOME`, a separate concern from this
//! adapter's write target) and registers its MCP servers in
//! **`~/.gemini/config/mcp_config.json`**'s top-level `mcpServers` map —
//! USER/GLOBAL scope, never a per-repo file. This adapter:
...
/// at construction, so `plan`/`apply`/`verify` all compute against the
/// exact same target. Tests point `home` at an isolated temp-dir fixture
/// instead of the real `~`.
#[derive(Debug, Clone)]
...
impl AntigravityAdapter {
    ...
    /// `~/.gemini/config/mcp_config.json` — the native Antigravity MCP
    /// registry, per the workpack's reference-format table.
    #[must_use]
...
impl HarnessAdapter for AntigravityAdapter {
    fn harness_key(&self) -> enforcer_domain::ids::HarnessId {
        enforcer_domain::ids::BuiltInHarness::Antigravity.id()
    }
        ...
        Ok(InstallReport {
            planned_changes: vec![PlannedInstallChange {
                ...
                description: InstallReportText::try_from(format!(
                    "upsert mcpServers[\"{SERVER_NAME}\"] in ~/.gemini/config/mcp_config.json \
...
#[cfg(test)]
mod tests {
    use super::{AntigravityAdapter, InstallError, SERVER_NAME};
    use crate::core::HarnessAdapter;
    use enforcer_domain::boundary::decode_error::DecodeError;
    ...
    #[test]
    fn harness_key_is_antigravity() -> Result<(), Box<dyn std::error::Error>> {
        let home = tempfile::tempdir()?;
        let adapter =
            AntigravityAdapter::try_new(home.path().to_path_buf(), home.path().join("enforcer"))?;
        assert_eq!(adapter.harness_key().as_str(), "antigravity");
        Ok(())
    }
    ...
    fn fresh_install_apply_then_verify_all_green() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let adapter = AntigravityAdapter::try_new(home.path().to_path_buf(), binary.clone())?;
    ...
    fn second_apply_is_byte_identical() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Idempotent re-install: second plan is a no-op, so nothing more
        // to apply -- but simulate a forced re-apply of the same desired
        // entry to prove the write itself is stable.
        let mut existing = adapter.read_config()?;
        AntigravityAdapter::merge_mcp_server(
            &mut existing,
            AntigravityAdapter::desired_entry(&binary),
        );
    ...
    fn verify_fails_when_entry_missing_or_renamed() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // not-yet-installed: missing entry
        ...
        // renamed key: install then rename the server entry
        let plan = adapter.plan(&ctx(&binary)?)?;
        adapter.apply(&plan)?;
