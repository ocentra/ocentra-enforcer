...
impl KiroAdapter {
    ...
    pub fn config_path(&self) -> PathBuf {
        self.home.join(".kiro").join("settings").join("mcp.json")
    }
...
mod tests {
    ...
    fn harness_key_is_kiro() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let adapter = KiroAdapter::new(home.path(), home.path().join("enforcer"));
        assert_eq!(adapter.harness_key(), "kiro");
        Ok(())
    }
    ...
    fn fresh_install_apply_then_verify_all_green() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let adapter = KiroAdapter::new(home.path(), &binary);
