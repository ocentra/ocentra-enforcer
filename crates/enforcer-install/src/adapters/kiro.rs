//! c09 — the Kiro [`crate::core::HarnessAdapter`].
//!
//! # Charter (workpack c09, reference-multiharness-install-matrix —
//! BINDING)
//!
//! Kiro's MCP registry lives under its home directory
//! (`~/.kiro`, matching c02 autodetect's `KIRO_HOME`/`.kiro` probe) at
//! **`~/.kiro/settings/mcp.json`** — USER/GLOBAL scope, never a per-repo
//! file. This adapter:
...
/// of `.kiro/settings/mcp.json`) and a `binary_path` fixed at
/// construction, so `plan`/`apply`/`verify` all compute against the exact
/// same target. Tests point `home` at an isolated temp-dir fixture
/// instead of the real `~`.
#[derive(Debug, Clone)]
...
impl KiroAdapter {
    ...
    /// `~/.kiro/settings/mcp.json` — the native Kiro MCP registry, per
    /// the workpack's reference-format table.
    #[must_use]
...
impl HarnessAdapter for KiroAdapter {
    fn harness_key(&self) -> enforcer_domain::ids::HarnessId {
        enforcer_domain::ids::BuiltInHarness::Kiro.id()
    }
        ...
        Ok(InstallReport {
            planned_changes: vec![PlannedInstallChange {
                ...
                description: InstallReportText::try_from(format!(
                    "upsert mcpServers[\"{SERVER_NAME}\"] in ~/.kiro/settings/mcp.json \
...
#[cfg(test)]
mod tests {
    use super::{InstallError, KiroAdapter, SERVER_NAME};
    use crate::core::HarnessAdapter;
    use enforcer_domain::boundary::decode_error::DecodeError;
    ...
    #[test]
    fn harness_key_is_kiro() -> Result<(), Box<dyn std::error::Error>> {
        let home = tempfile::tempdir()?;
        let adapter =
            KiroAdapter::try_new(home.path().to_path_buf(), home.path().join("enforcer"))?;
        assert_eq!(adapter.harness_key().as_str(), "kiro");
        Ok(())
    }
    ...
    fn fresh_install_apply_then_verify_all_green() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let adapter = KiroAdapter::try_new(home.path().to_path_buf(), binary.clone())?;
    ...
    fn second_apply_is_byte_identical() -> Result<(), Box<dyn std::error::Error>> {
        ...
        KiroAdapter::merge_mcp_server(&mut existing, KiroAdapter::desired_entry(&binary));
    ...
    fn upsert_preserves_unrelated_keys_and_servers() -> Result<(), Box<dyn std::error::Error>> {
        let home = tempfile::tempdir()?;
        let binary = home.path().join("bin").join("enforcer");
        let config_dir = home.path().join(".kiro").join("settings");
    ...
    fn malformed_config_is_a_detected_plan_failure() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let config_dir = home.path().join(".kiro").join("settings");
        fs::create_dir_all(&config_dir)?;
        fs::write(config_dir.join("mcp.json"), "{ not valid json")?;
        let adapter = KiroAdapter::try_new(home.path().to_path_buf(), binary.clone())?;
