//! c09 — the Windsurf [`crate::core::HarnessAdapter`].
//!
//! # Charter (workpack c09, reference-multiharness-install-matrix —
//! BINDING)
//!
//! Windsurf speaks the same `mcpServers` family as Cursor, at
//! **`~/.codeium/windsurf/mcp_config.json`** — USER/GLOBAL scope, never a
//! per-repo file. This adapter:
...
/// the exact same target. Tests point `home` at an isolated temp-dir
...
mod tests {
    ...
    fn second_apply_is_byte_identical() -> Result<(), Box<dyn std::error::Error>> {
        ...
        WindsurfAdapter::merge_mcp_server(&mut existing, WindsurfAdapter::desired_entry(&binary));
