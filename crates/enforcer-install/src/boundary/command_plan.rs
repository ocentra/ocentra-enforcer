...
//! surface, and never a dispatch that short-circuits to a fixed/fabricated
//! result.
//!
...
//! Negative invalid inputs are rejected during command decoding.
//!
...
/// the mechanical real-dispatch check both the Claude and generic renderers
/// must satisfy. A dispatch that hardcodes a fixed/fabricated result, or that
...
mod tests {
    ...
    fn claude_command_carries_frontmatter_and_description() {
        ...
        assert!(rendered.as_str().starts_with("---\n"));
        assert!(rendered
            .as_str()
            .contains(&format!("description: {DESCRIPTION}")));
        assert!(rendered.as_str().contains("# /plan"));
    }
    ...
    fn generic_command_has_no_frontmatter_fence() {
        ...
        assert!(!rendered.as_str().starts_with("---\n"));
        assert!(rendered.as_str().starts_with("# /plan"));
        assert!(rendered.as_str().contains(DESCRIPTION));
    }
    ...
    fn command_body_documents_both_the_scaffold_and_validate_steps() {
        let body = render_command_body();
        assert!(body.as_str().contains("plan new"));
        assert!(body.as_str().contains("plan check"));
        assert!(body.as_str().contains("SKILL.md"));
    }
}
