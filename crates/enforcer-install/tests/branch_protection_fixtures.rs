...
use enforcer_install::ci::boundary::branch_protection::{
    report, BranchProtectionWriteDto, LiveProtectionStateDto, WorkflowJobDeclaration,
};
use enforcer_install::ci::branch_protection::{verify, DesiredProtection, Verification};
...
fn desired() -> Result<DesiredProtection, Box<dyn std::error::Error>> {
    ...
    Ok(DesiredProtection::baseline(rust_ci.union(&enforcer).cloned().collect()))
}

fn live(name: &str) -> Result<ObservedBranchProtection, Box<dyn std::error::Error>> {
    let raw = std::fs::read_to_string(fixture_path(name))?;
    Ok(ObservedBranchProtection::try_from(serde_json::from_str::<LiveProtectionStateDto>(&raw)?)?)
}
...
fn github_live_fixture_maps_to_typed_observation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(observed.context_requirement(&context), ContextRequirement::Present);
...
fn fixture_policy_attests_only_the_safe_observation() -> Result<(), Box<dyn std::error::Error>> {
    let desired = desired()?;
    assert!(matches!(verify(&desired, &live("pass.json")?), Verification::Attested));
    assert!(matches!(verify(&desired, &live("fail_no_required_checks.json")?), Verification::Refused(_)));
    assert!(matches!(verify(&desired, &live("fail_bypassable.json")?), Verification::Refused(_)));
    assert!(matches!(verify(&desired, &live("fail_red_merge_eligible.json")?), Verification::Refused(_)));
    Ok(())
}
