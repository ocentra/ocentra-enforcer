//! The consolidated MCP tool registry: every tool this server exposes,
...
/// One MCP tool's static descriptor.
...
/// reproducible).
pub fn build_tool_descriptors() -> Vec<ToolDescriptor> {
    let mut out: Vec<ToolDescriptor> = CANONICAL_TOOLS
        .iter()
    ...
    if crate::aliases::deprecation_window_open() {
        for &canonical in CANONICAL_TOOLS {
            let alias = crate::aliases::alias_name(canonical);
