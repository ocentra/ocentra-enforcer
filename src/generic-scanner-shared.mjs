...
/** Defines the generic scanner's registered rule metadata. */
...
const weakAssertionPatterns = [
  ...
  },
  {
    ...
    isWeak: (line) => !hasExplicitContainsExpectation(line),
  },
  {
...
function hasExplicitContainsExpectation(line) {
  const match = line.match(/\.contains\(\s*(?<expected>.+?)\s*\)(?:\s*[,);])/u);
  const expected = match?.groups?.expected ?? "";
  return /(?:r#+)?["'][^"']+["']|\b\d+(?:\.\d+)?\b|\b[A-Z][A-Za-z0-9_]*::[A-Z][A-Za-z0-9_]*\b|\b[A-Z][A-Z0-9_]{2,}\b/u.test(expected);
}
...
const placeholderCommentPatterns = [
  ...
  {
    pattern: new RegExp(String.raw`^\s*(?://+|/\*+|\*+|#)\s*(?:a\s+)?${word("place", "holder")}\b`, "iu"),
    detail: "placeholder marker found in production source.",
  },
  {
    pattern: new RegExp(String.raw`^\s*(?://+|/\*+|\*+|#)\s*(?:a\s+)?${word("st", "ub")}\b`, "iu"),
    detail: "stub marker found in production source.",
  },
  {
    pattern: new RegExp(String.raw`^\s*(?://+|/\*+|\*+|#)\s*(?:a\s+)?${word("fa", "ke")}\b`, "iu"),
    detail: "fake marker found in production source.",
  },
  {
    pattern: /^\s*(?:\/\/+|\/\*+|\*+|#)\s*temporary\b/iu,
    detail: "temporary marker found in production source.",
  },
  {
    pattern: /^\s*(?:\/\/+|\/\*+|\*+|#)\s*for now\b/iu,
    detail: "for now marker found in production source.",
  },
  {
    pattern: /^\s*(?:\/\/+|\/\*+|\*+|#)\s*scaffold[- ]only\b/iu,
...
function firstDuplicateFunctionName(lines) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const line = lines[index] ?? "";
    const cfgMatch = line.match(/^\s*#\s*\[\s*cfg\s*\((.+)\)\s*\]\s*$/u);
    if (cfgMatch) {
      pendingCfg = cfgMatch[1]?.trim() ?? "";
      continue;
    }
    const match = line.match(pattern);
    const moduleDepth = moduleScopes.at(-1)?.depth ?? 0;
    if (match && braceDepth === moduleDepth) {
      const name = match[1];
      const scope = moduleScopes.map((entry) => entry.name).join("::");
      const key = `${scope}\0${name}`;
      const prior = seen.get(key) ?? [];
      if (prior.some((entry) => entry.cfg === "" || pendingCfg === "" || entry.cfg === pendingCfg)) {
        return { name, line: index + 1, source: line };
      }
      prior.push({ cfg: pendingCfg, line: index + 1 });
      seen.set(key, prior);
      pendingCfg = "";
    }
    const structuralLine = line
      .replace(/r(?<hashes>#{0,8})"[\s\S]*?"\k<hashes>/gu, '""')
      .replace(/"(?:\\.|[^"\\])*"/gu, '""')
      .replace(/'(?:\\.|[^'\\])'/gu, "''")
      .replace(/\/\/.*$/u, "");
    const lineBraceDelta =
      (structuralLine.match(/\{/gu) ?? []).length - (structuralLine.match(/\}/gu) ?? []).length;
    braceDepth += lineBraceDelta;
    const moduleMatch = line.match(/^\s*(?:pub(?:\([^)]*\))?\s+)?mod\s+([A-Za-z_]\w*)\s*\{/u);
    if (moduleMatch && lineBraceDelta > 0) {
      moduleScopes.push({ name: moduleMatch[1], depth: braceDepth });
    }
    while (moduleScopes.length > 0 && braceDepth < moduleScopes.at(-1).depth) {
      moduleScopes.pop();
    }
    if (line.trim() !== "" && !/^\s*#\s*\[/u.test(line) && !match) {
      pendingCfg = "";
    }
  }
  return null;
