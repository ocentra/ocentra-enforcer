...
const CORE_RULES = [
  ...
  { ruleId: 'TS-6.7', label: 'raw string domain alias found', pattern: nakedDomainAliasPattern, domainOnly: true },
  { ruleId: 'TS-6.8', label: 'raw number domain value found', pattern: rawNumberDomainPattern, domainOnly: true },
  { ruleId: 'TS-6.9', label: 'raw boolean domain parameter found', pattern: rawBooleanDomainPattern, domainOnly: true },
  { ruleId: 'TS-6.10', label: 'Record<string, domain> API found', pattern: recordStringDomainPattern, domainOnly: true },
  { ruleId: 'TS-6.11', label: 'Map<string, domain> API found', pattern: mapStringDomainPattern, domainOnly: true },
  { ruleId: 'TS-6.12', label: 'string[] domain API found', pattern: stringArrayDomainPattern, domainOnly: true },
  ...
  { ruleId: 'TS-6.20', label: 'Date found in domain API', pattern: dateDomainPattern, domainOnly: true },
  ...
  { ruleId: 'TS-6.26', label: 'return null found in domain API', pattern: returnNullPattern, domainOnly: true },
  { ruleId: 'TS-6.27', label: 'undefined domain state found', pattern: undefinedStatePattern, domainOnly: true },
  {
    ruleId: 'TS-6.28',
  ...
  { ruleId: 'TS-6.29', label: 'Partial<T> domain logic found', pattern: partialPattern, domainOnly: true },
  { ruleId: 'TS-6.30', label: 'Record<string, unknown> payload found', pattern: recordUnknownPayloadPattern, domainOnly: true },
  { ruleId: 'TS-6.32', label: 'dynamic import found in domain code', pattern: dynamicImportPattern, domainOnly: true },
  { ruleId: 'TS-6.34', label: 'dynamic code execution found', pattern: dynamicCodePattern },
  { ruleId: 'TS-6.35', label: 'raw DTO spread found', pattern: rawDtoSpreadPattern, domainOnly: true },
  { ruleId: 'TS-6.36', label: 'any spread into domain object found', pattern: anySpreadPattern, domainOnly: true },
  { ruleId: 'TS-6.40', label: 'mutating shared object found', pattern: sharedMutationPattern, domainOnly: true },
];

function maskJsxText(line) {
  if (!/<\/?[A-Za-z][A-Za-z0-9:._-]*(?:\s|>|\/)/u.test(line)) return line;
  return line
    .replace(/>[^<{]*(?=\{)/gu, '>')
    .replace(/([>}])[^<{]*(?=<)/gu, '$1');
}
...
function scanRuleSet(root, filePath, lines, rules) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    for (const rule of rules) {
      ...
      if (rule.domainOnly && !isDomainLikeTypeScriptPath(rel)) continue;
      const candidate = rule.useRawLine
        ? line
        : rule.ruleId === 'TS-6.3'
          ? maskJsxText(masked)
          : masked;
...
/** Scans TypeScript domain-core source for policy violations. */
