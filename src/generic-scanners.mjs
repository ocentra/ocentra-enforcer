...
export function runGenericScan({ root, scope, config, languages = [], sourceOnly = false }) {
  const activeLanguages = new Set(languages);
  const files = collectGenericScopeFiles(root, scope, config, activeLanguages, sourceOnly);
  const violations = [];
