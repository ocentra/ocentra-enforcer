...
const SECRET_RE =
  /\b(?:api[_-]?key|secret|token|password|private[_-]?key)\b\s*[:=]\s*["'][A-Za-z0-9_./+=:@-]{16,}["']/iu;
...
const weakAssertionPatterns = [
  ...
  {
    pattern: /assert!\(\s*[\w.]+\s*\.is_some\(\)\s*\)/u,
    detail: "assert!(value.is_some()) is too weak.",
  },
  {
    pattern: /assert!\(\s*[\w.]+\s*\.is_ok\(\)\s*\)/u,
    detail: "assert!(result.is_ok()) is too weak.",
  },
  {
    pattern: /assert!\(\s*[\w.]+\s*\.is_err\(\)\s*\)/u,
