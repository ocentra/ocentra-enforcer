...
const exportedRuleIdAlias = assemble("export type RuleId = str", "ing;");
...
${manualBrandAlias}
${tsIgnoreComment}
const apiKey = "${"sk"}_${"test"}_1234567890abcdef";
`,
    "src/generated.ts": `
// @generated by a tool
...
generic_secret = "${"abcdefghijklmnopqrstuvwxyz"}${"ABCDEFGHIJKLMNOPQRSTUVWXYZ"}1234567890"
...
test("TypeScript domain rules do not classify presentation and tooling boundaries as domain APIs", () => {
  const project = makeProject({
    "src/ui/component.tsx": `
type ViewPayload = { itemCount: number; enabled: boolean; tags: string[] };
export function View(): JSX.Element {
  const payload = { nodes: [1, 2] };
  const nodes = [...payload.nodes];
  const describe = (value: unknown): string => String(value);
  return <small>{describe(nodes.length)} values are rendered as a list</small>;
}
`,
...
`,
    "packages/app/tests/browser-proof.mjs": `
export async function browser() {
  return import("playwright");
}
`,
    "packages/app/src/bindings/RuleId.ts": `
// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs). Do not edit this file manually.
/** Rule identifier, as its wire string. */
${exportedRuleIdAlias}
`,
  });
  const result = run(project, [
  ...
  assert.deepEqual(
    violations.filter((violation) => forbidden.has(violation.ruleId)),
    [],
  );
});
...
test("boundary decision detection ignores comment prose but retains executable decisions", () => {
  const project = makeProject({
    "ocentra-enforcer.config.json": JSON.stringify({
      schemaVersion: 2,
      profileName: "strict",
    ...
    "src/boundary/commented.ts": `
/** BOUNDARY-INVARIANT: invalid raw input is rejected.
 * Documentation may explain what happens if policy data is malformed.
 */
export function decodeCommented(value: unknown) {
  return value;
}
`,
    "src/boundary/actual.ts": `
/** BOUNDARY-INVARIANT: invalid raw input is rejected. */
export function decodeActual(value: unknown, policy: unknown) {
...
`,
  ...
  const violations = JSON.parse(result.stdout).violations
    .filter((violation) => violation.ruleId === "BOUND-1.3");
  assert.equal(
    violations.some((violation) => violation.file.endsWith("src/boundary/actual.ts")),
    true,
  );
  assert.equal(
    violations.some((violation) => violation.file.endsWith("src/boundary/commented.ts")),
    false,
  );
});

test("common scanner accepts semantic boundary docs and does not require a waiver for a small boundary", () => {
  const project = makeProject({
    "ocentra-enforcer.config.json": JSON.stringify({
      schemaVersion: 2,
      profileName: "strict",
    ...
    "package.json": JSON.stringify({ name: "boundary-docs-fixture", version: "0.0.0" }),
    "src/adapters/recorded.ts": `
/** Wire boundary: parse raw adapter input, reject malformed values, and map accepted data to domain values. */
type InputDto = { value: string };
type DomainRecord = { value: string };
export function parseInput(raw: InputDto): DomainRecord {
...
`,
  ...
  assert.doesNotMatch(output, /BOUND-1\.(?:1|5|7)/u, output);
});
