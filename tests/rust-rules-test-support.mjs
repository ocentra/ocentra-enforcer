...
export { DEFAULT_CONFIG, normalizeConfig, runScanner };
...
export function expectFailure(project, ruleId) {
  ...
  assert.match(
    output,
    new RegExp(ruleId.replace('.', '\\.'), 'u'),
    `expected output to contain ${ruleId}. Output:\n${output}`,
  );
  ...
  assert.match(
    output,
    /rules\/rust\//u,
    'failure output must point at indexed rules doc',
  );
}
...
export function expectFailures(project, ruleIds) {
  ...
  assert.notEqual(
    result.status,
    0,
    `expected gate to fail for ${ruleIds.join(', ')}`,
  );
  const output = `${result.stdout}\n${result.stderr}`;
...
export function expectNoRule(project, ruleId) {
  const result = runGate(project);
  const escapedRuleId = ruleId.replace('.', '\\.');
  assert.doesNotMatch(
    `${result.stdout}\n${result.stderr}`,
    new RegExp(`${escapedRuleId}(?!=[0-9])`, 'u'),
  );
}
