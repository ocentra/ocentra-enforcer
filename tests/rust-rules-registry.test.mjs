...
test('registry has the expected schema shape', () => {
  ...
  assert.deepEqual(registry.languages, ['rust', 'typescript', 'python', 'common']);
  assert.ok(Array.isArray(registry.rules));

  for (const rule of registry.rules) {
    assert.match(rule.id, /^(RR|TS|PY|SEC|GEN|DOC|HAR|TEST|PORT|SRC|CONTRACT|DEP|SBOM|AI)-[0-9]+\.[0-9]+$/u);
    assert.ok(['rust', 'typescript', 'python', 'common'].includes(rule.language));
    assert.ok(
      [
        'source',
        'domain',
...
test('registry exactly covers enforced scanner rules', () => {
  ...
  assert.deepEqual(registryIds, [...scannerRuleIds(), ...Object.keys(GENERIC_RULES), ...Object.keys(CHECK_RULES)].sort());
});
