...
export function collectTestFunctions(source, masked = source) {
  const functions = [];
  for (const attribute of masked.matchAll(/^\s*#\s*\[\s*test\s*\]\s*$/gmu)) {
    ...
    functions.push({ start: attribute.index, bodyStart: openingBrace + 1, bodyEnd: Math.max(openingBrace + 1, end - 1) });
  }
  return functions;
}
