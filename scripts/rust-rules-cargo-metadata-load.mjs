...
/** Loads Cargo metadata without permitting Cargo.lock mutation. */
export function loadLockedCargoMetadata(root) {
  // First ask Cargo to resolve the locked workspace so a stale lock is still
  // a hard error. Then load only direct workspace packages for policy.
  ...
  if (direct.metadata || direct.unavailable) return direct;
  if (cargoLockNeedsUpdate(direct.output)) return direct;
